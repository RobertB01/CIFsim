...
public class NoSpecificUnaryExprsCheck extends CifCheck {
        ...
        switch (op) {
            ...
            case NEGATE:
                if (disallowNegate) {
                    ...
                    } else if (disallowNegateIntsRangeless) {
                        CifType ctype = CifTypeUtils.normalizeType(unExpr.getChild().getType());
            ...
            case PLUS:
                if (disallowPlus) {
                    ...
                    } else if (disallowPlusIntsRangeless) {
                        CifType ctype = CifTypeUtils.normalizeType(unExpr.getChild().getType());
                        if (ctype instanceof IntType && CifTypeUtils.isRangeless((IntType)ctype)) {
                            addExprViolationOperand(unExpr);
                        }
    ...
    private void addExprViolationOperator(UnaryExpression unExpr) {
        super.addViolation(getNamedAncestorOrSelf(unExpr),
                fmt("unary operator \"%s\": \"%s\"", operatorToStr(unExpr.getOperator()), exprToStr(unExpr)));
    }
    ...
    private void addExprViolationOperand(UnaryExpression unExpr) {
        super.addViolation(getNamedAncestorOrSelf(unExpr), fmt("unary operator \"%s\" on operand \"%s\"",
                operatorToStr(unExpr.getOperator()), exprToStr(unExpr.getChild())));
    }
}
