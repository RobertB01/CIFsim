...
This class is also used for bodies of group definitions.
...
\citemnf{Invariant.eventInAlphabet}
  The event, if set, should refer to an event that is in the alphabet of the
  specification, considering the synchronization, send and receive alphabets.
\end{constraints}
}
...
This class is also used for algebraic parameters of component definitions.
...
\citem{AlgVariable.allowedTypes}
  ...
  variables.
\citemnf{AlgVariable.typeNotSingleValue}
  The \emph{type} should not allow only a single value, as then a constant
  should be used instead (for algebraic variables), or a parameter is not
  needed (for algebraic parameters).
\end{constraints}
}
...
\citem{DiscVariable.allowedTypes}
  ...
  variables.
\citemnf{DiscVariable.typeNotSingleValue}
  The \emph{type} should not allow only a single value, as then a constant
  should be used instead (for discrete variables and local variables of
  functions), or a parameter is not needed (for function parameters).
\end{constraints}
}
...
This class is also used for event parameters of component definitions.
...
\citemnf{Event.typeNotSingleValue}
  The \emph{type} should not allow only a single value, as then a constant
  should be used instead, rather than communicating the value via a channel.
\end{constraints}
}
...
\citemnf{InputVariable.typeNotSingleValue}
  The \emph{type} should not allow only a single value, as then a constant
...
\citem{Assignment.addressableSyntax}
  ...
  string typed variables are not allowed. Assignments to derivatives of continuous
...
This class is also used for bodies of automaton definitions.
...
This class is also used for location parameters of component definitions.
}
...
exclude positive and negative infinities, as well as NaN values, and any
...
This class is used both to refer to algebraic variables and to algebraic
parameters of component definitions.
...
This class is used to refer to automata and groups. It is not used to refer
to the bodies of component definitions.
...
This class is used to refer to discrete variables, as well as to refer to
parameters and local variables of internal user-defined functions.
...
This class is used both to refer to events and to event parameters of
...
This class is used both to refer to locations and to location parameters of
component definitions.
...
\citem{Function.allowedReturnTypes}
  ...
  be used as values.
\citemnf{Function.returnTypeNotSingleValue}
  The \emph{returnTypes} should not allow only a single value, as then a
  constant should be used instead of a function.
\end{constraints}
}
...
represents a single argument of an annotation. And the
...
\citem{AnnotatedObject.onlyForSupportedObjects}
  The following objects support annotations:
  \begin{itemize}
    \item Algebraic parameters (for which we reuse the \cifclass{AlgVariable}
      class).
    \item Algebraic variables (includes algebraic parameters).
    \item Automata (including bodies of automaton definitions).
    \item Automaton definitions (for which we reuse the \cifclass{Automaton}
      class).
    \item Automaton instantiations.
    \item Constants.
    \item Continuous variables.
    \item Discrete variables (including parameters of user-defined functions,
      and local variables of internal user-defined functions).
    \item Edges.
    \item Enumeration literals.
    \item Enumerations.
    \item Events.
    \item Function parameters (for which we reuse the \cifclass{DiscVariable}
      class).
    \item Function variables (for which we reuse the \cifclass{DiscVariable}
      class).
    \item Functions.
    \item Group definitions (for which we reuse the \cifclass{Group} class).
    \item Group instantiations.
    \item Groups (including bodies of group definitions).
...
\citem{Annotation.uniqueArguments}
  The names of each of the named arguments must be unique within the annotation.
\end{constraints}
}
...
The optional name of the annotation argument.

\begin{constraints}
\citem{AnnotationArgument.validName}
  If the argument has a name, it must be a CIF identifier, or multiple CIF
  identifiers joined together using periods (\texttt{.}). For instance,
  \texttt{arg} or \texttt{some.arg}.
\end{constraints}
}
