...
definitions (\cifclass{ComponentDef} class) and their formal parameters.
}
...
A formal algebraic (variable) parameter of a \cifclass{ComponentDef}.

Note that there are no constant parameters. However, constants may be provided
as actual arguments for formal algebraic parameters.
}

\newcommand{\featdocuAlgParametervariable}{
The algebraic variable declaration that represents the formal algebraic
parameter.

\begin{constraints}
\citem{AlgParameter.noValue}
  The \emph{variable} declaration must not have a value, since the actual
  parameter should provide it.
\end{constraints}
}
...
\citem{ComponentDef.uniqueDecls}
  The names of all parameters and declarations in the body of the component
  definition, must be unique within that component definition. For
  bodies that are automata, we have to take into account the names of all
  declarations (including locations) of those automata. For bodies that are
  groups, we have to take into account the names of all declarations,
  child components, and component definitions, of those groups.
\end{constraints}
}
...
The actual parameters (or arguments), to match each of the formal parameters
of the component definition.

\begin{constraints}
\citem{ComponentInst.parameterCount}
  The number of formal parameters must be equal to the number of actual
  parameters.
\citem{ComponentInst.parameterTypes}
  The type of each actual parameter must match the type of the
  corresponding formal parameter. That is:
  \begin{itemize}
  \item For each formal component parameter, the actual parameter must be a
    reference to a matching component. That is, for
    each formal parameter with its component definition type, the actual
    parameter must be a reference to a component that is an instance of that
    component definition.
  \item For each formal event parameter, the actual parameter must be a
    reference to an event.

    If the formal parameter specifies the controllability of the event, the
    event used as actual parameter must have the same controllability as the
    formal parameter. If the formal parameter does not specify the
    controllability, the controllability of the actual argument does not
    matter (it may have controllability, but it may also not have
    controllability).

    If the formal parameter specifies the type of the event, the event used as
    actual parameter must have an equal type (including ranges). If the formal
    parameter does not specify the type of the event, the type of the event
    used as actual parameter does not matter (it may have a type, but it may
    also not have a type).

    If the formal parameter specifies event usage restriction flags, the event
    used as actual parameter must support all those usages. If the formal
    parameter specifies any flags, it requires that the event used as actual
    argument supports at least those usages. If the formal parameter does not
    specify any flags, it requires that the event used as actual argument
    supports all usages. If the event used as actual argument is an event
    declaration, it automatically supports all usages. If the event used as
    actual argument is an event parameter, its usage restriction flags should
    be checked. If the actual event parameter does not have any flags, it
    supports all usages. If the actual event parameter does have one or more
    flags, it supports only those usages specified by the flags.

    It is not possible to use the `tau' event as actual parameter.
  \item For each formal location parameter, the actual parameter must be a
    reference to a location.
  \item For each formal algebraic parameter, the actual parameter must be an
    expression with a compatible type. If the formal parameter type has a
    range, the actual parameter type must have one as well, and the range of
    the type of the actual parameter must be entirely contained in the range of
...
A formal component parameter of a \cifclass{ComponentDef}.
}

\newcommand{\featdocuComponentParametername}{
The name of the formal component parameter. Note that other formal parameters
get their name via the contained declaration. Since the formal component
...
The type of components that may be passed along as actual parameter.
...
A formal event parameter of a \cifclass{ComponentDef}.
...
The event declaration that represents the formal event parameter.
}
...
\citem{Group.uniqueDecls}
  The names of all declarations, child components, and component definitions
  defined in a group must be unique within that group.
\end{constraints}
}
...
An invariant, with optional supervisory kind. Can either represent a state
(exclusion) invariant, or a state/event exclusion invariant.
}
...
The supervisory kind of the invariant. If \cifenumlit{SupKind.None}, and the
invariant is specified in an automaton or a location of an automaton, the
invariant implicitly inherits the automaton supervisory kind. If not specified,
and the invariant is not specified in an automaton or a location of an
automaton, the invariant has no kind (\cifenumlit{SupKind.None}).

\begin{constraints}
\citemnf{Invariant.inherited}
  The invariant must not have a supervisory kind inherited from the automaton.
  Kind inheritance is deprecated, as this can be confusing for end users. To
  get rid of the warning, specify the supervisory kind explicitly.
\end{constraints}
}
...
A formal location parameter of a \cifclass{ComponentDef}.
}

\newcommand{\featdocuLocationParameterlocation}{
The location declaration that represents the formal location parameter.
...
A formal parameter of a \cifclass{ComponentDef}.
}
...
\citem{AlgVariable.uniqueValue}
  ...
  must not be set, since then the actual parameter should provide the value.
...
Furthermore, the class is used for local variables for the receives on edges
(the so-called `receive variables').
...
\citem{DiscVariable.occurrence}
  ...
  They may also be used in function parameters, as local variables in
  functions, and as receive variables, but then they are usually not called
  discrete variables.
\end{constraints}
}
...
If specified, indicates whether the event is controllable or uncontrollable.
}
...
`old' state. Note that the `receive variable', if any, evaluates to the
received value.

\begin{constraints}
\citem{Assignment.addressableSyntax}
  Adressables may be discrete and continuous variable references (non-wrapped),
  with projections, and may optionally be wrapped in tuples (possibly multiple
  times). Projected string typed variables are not allowed.
\citem{Assignment.variablesInScope}
  The variables that are assigned must be a discrete or continuous variables
  declared in the automaton that contains the source location of the edge of
  this assignment update. This does not include the `receive variable', if any.
  Discrete and continuous variables may be mixed in a single assignment.
\end{constraints}
}
...
This expression is evaluated in the `old' state. Note the `receive variable',
if any, evaluates to the received value.
}
...
\citem{Automaton.uniqueDecls}
  The names of all declarations and locations defined in an automaton must
  be unique within that automaton.
...
It is not possible to use `receive variables' in the guards (as they are not
`in scope').
...
\citem{Edge.uniqueVariables}
  ...
  Note that `receive variables' may never be assigned in the updates. See
  also the \emph{Assignment.variablesInScope} constraint.
\end{constraints}
}
...
The formal component parameter via which the object is referenced.
...
The object that is referenced via a formal component parameter.
...
Greater or equal comparison binary operator ($\geq$).
}
...
Less than comparison binary operator ($\leq$).
}
...
\citem{FunctionCallExpression.type}
  ...
  and the parameters. That is, the function must have a function type. The
  ...
  The count and types of the parameters must match the parameter types of the
  function type. For ranged types, the ranges (if specified) of the parameters
...
\citem{FunctionExpression.type}
  ...
  parameter types matches the parameter types of the referenced function.
  For ranged types, the ranges must match exactly.
\end{constraints}
}
...
\citem{ReceivedExpression.scope}
  ...
  value is received. Received variables may not be assigned (are read-only).
...
\citem{StdLibFunctionExpression.type}
  ...
  \cifclass{StdLibFunction}, and for some functions, also on the parameters.
  Tables~\ref{tbl:stdlib-types}, \ref{tbl:stdlib-trig-types}, and
  \ref{tbl:stdlib-dist-types} list the allowed combinations.

  For integer types, if one of the parameters has a rangeless integer type,
  the result type is also rangeless, if the result type is an integer type as
...
\citem{SwitchCase.keyType}
  ...
  not refer to an automaton, the type of the `key' expression must be contained
...
\citem{AssignmentFuncStatement.variablesInScope}
  The variables that are assigned must a local variables declared in the
...
\citem{FunctionParameter.noValue}
  The \emph{parameter} declaration must not have a value, since the actual
  parameter should provide it.
\end{constraints}
}
...
use both \texttt{/} and \texttt{\textbackslash} as path separators. If
...
\texttt{\textbackslash} as path separators. If relative, the path is relative
