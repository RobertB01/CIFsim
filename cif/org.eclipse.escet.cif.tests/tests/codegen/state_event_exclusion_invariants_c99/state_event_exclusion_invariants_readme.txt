The 'state_event_exclusion_invariants_*' files here are generated using the CIF to C99 code generator.
Below is a description of what it contains, and how to use and/or tailor them
towards your wishes.

Generated files
===============
* state_event_exclusion_invariants_library.c and state_event_exclusion_invariants_library.h
...
* state_event_exclusion_invariants_engine.c and state_event_exclusion_invariants_engine.h
...
* state_event_exclusion_invariants_compile.sh
  Simple compile script to compile everything with gcc in C99.

* state_event_exclusion_invariants_test_code.c
  Example usage and test compile code.

* state_event_exclusion_invariants_readme.txt
  This file.
...
* state_event_exclusion_invariants_AssignInputVariables(void)
...
* state_event_exclusion_invariants_InfoEvent(state_event_exclusion_invariants_Event_ event, BoolType pre)
  Announcement or report of an event taking place. Can be disabled with a
  compile-time flag.

* state_event_exclusion_invariants_PrintOutput(const char *line, const char *fname)
...
* state_event_exclusion_invariants_EngineFirstStep(void)
  Initializes the internal data, reads the input variables, and performs
  events until the first time delay step.

* state_event_exclusion_invariants_EngineTimeStep(double delta)
...
* EVENT_OUTPUT
  ...
  external "state_event_exclusion_invariants_InfoEvent" function. There is no CIF equivalent of this
  function.

* PRINT_OUTPUT
  If true (-DPRINT_OUTPUT=1), print statements are performed as specified in
  CIF. The results are routed to the external "state_event_exclusion_invariants_PrintOutput" function.
