The 'exprs_*' files here are generated using the CIF to C89 code generator.
Below is a description of what it contains, and how to use and/or tailor them
towards your wishes.

Generated files
===============
* exprs_library.c and exprs_library.h
...
* exprs_engine.c and exprs_engine.h
...
* exprs_AssignInputVariables(void)
...
* exprs_InfoEvent(exprs_Event_ event, BoolType pre)
  Announcement or report of an event taking place. Can be disabled with a
  compile-time flag.

* exprs_PrintOutput(const char *line, const char *fname)
...
* EVENT_OUTPUT
  ...
  external "exprs_InfoEvent" function. There is no CIF equivalent of this
  function.

* PRINT_OUTPUT
  If true (-DPRINT_OUTPUT=1), print statements are performed as specified in
  CIF. The results are routed to the external "exprs_PrintOutput" function.
