/* Simulink S-Function code for locs_multiple_potential_initial CIF file.
 *
...
#define S_FUNCTION_NAME locs_multiple_potential_initial
...
static int EnumTypePrint(locs_multiple_potential_initialEnum value, char *dest, int start, int end);
...
static void mdlInitializeSizes(SimStruct *sim_struct) {
    ...
    /* Outputs. */
    if (!ssSetNumOutputPorts(sim_struct, 2)) return;
...
static void mdlInitializeConditions(SimStruct *sim_struct) {
    ...
    modes[0] = _locs_multiple_potential_initial_loc1;
}
#endif
/* }}} */
...
static void mdlOutputs(SimStruct *sim_struct, int_T tid) {
    ...
    *y = BoolToSimulink(a_x_(sim_struct));
}
/* }}} */
