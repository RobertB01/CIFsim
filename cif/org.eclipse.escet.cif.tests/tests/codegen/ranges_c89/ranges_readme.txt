The 'ranges_*' files here are generated using the CIF to C89 code generator.
Below is a description of what it contains, and how to use and/or tailor them
towards your wishes.

Generated files
===============
* ranges_library.c and ranges_library.h
...
* ranges_engine.c and ranges_engine.h
...
* ranges_AssignInputVariables(void)
...
* ranges_InfoEvent(ranges_Event_ event, BoolType pre)
  Announcement or report of an event taking place. Can be disabled with a
  compile-time flag.

* ranges_PrintOutput(const char *line, const char *fname)
...
* EVENT_OUTPUT
  ...
  external "ranges_InfoEvent" function. There is no CIF equivalent of this
  function.

* PRINT_OUTPUT
  If true (-DPRINT_OUTPUT=1), print statements are performed as specified in
  CIF. The results are routed to the external "ranges_PrintOutput" function.
