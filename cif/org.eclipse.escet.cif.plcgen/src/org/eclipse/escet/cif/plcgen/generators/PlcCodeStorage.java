...
public class PlcCodeStorage {
    ...
    public void finishPlcProgram() {
        ...
        if (stateInitializationCode != null) {
            ...
            //
            box.add("IF %s THEN", firstRun.varName);
            box.indent();
            box.add("%s := FALSE;", firstRun.varName);
            if (loopsKilled != null) {
                box.add("%s := 0;", loopsKilled.varName);
            }
            box.add();
        ...
        if (eventTransitionsIterationCode != null) {
            ...
            if (loopCount == null) {
                ...
                box.add("%s := 0;", loopCount.varName);
                box.add("WHILE %s DO", textGenerator.toString(whileCond));
                box.indent();
                box.add("%s := %s + 1;", loopCount.varName, loopCount.varName);
            }

            // Construct the while body with event processing.
            box.add("%s := FALSE;", progressVar.varName);
            box.add();
            ...
            if (loopsKilled != null) {
                ...
                box.indent();
                box.add("%s := %s;", loopsKilled.varName, textGenerator.toString(limitedIncrementKilled));
                box.dedent();
                box.add("END_IF;");
            }
        }
