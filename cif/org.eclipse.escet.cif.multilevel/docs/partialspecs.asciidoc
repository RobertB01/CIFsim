...
| `DiscVariableExpression` | `DiscVariable` | `DiscVariable` is also copied if its containing class in the partial specification is an `Automaton`, else it is converted to an `InputVariable` with the same domain is done.
