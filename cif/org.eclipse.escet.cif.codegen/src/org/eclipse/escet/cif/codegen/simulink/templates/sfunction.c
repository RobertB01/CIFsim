...
static void mdlInitializeSizes(SimStruct *sim_struct) {
    ...
    /* Inputs. */
    if (!ssSetNumInputPorts(sim_struct, ${number-of-inputs})) return;

${set-input-ports-dimensions}

    int idx;
    for (idx = 0; idx < ${number-of-inputs}; idx++) {
    ...
    /* Outputs. */
    if (!ssSetNumOutputPorts(sim_struct, ${number-of-outputs})) return;

${set-output-ports-dimensions}

    for (idx = 0; idx < ${number-of-outputs}; idx++) {
    ...
    ssSetNumContStates(sim_struct, ${number-of-contvars}); /* CState[0] is time. */
    ...
    /* Modes. */
    ssSetNumModes(sim_struct, ${number-of-modes});

    ssSetNumSampleTimes(sim_struct, 1);
    ssSetNumNonsampledZCs(sim_struct, ${number-of-time-dependent-guards});

    ssSetOptions(sim_struct, 0);
}

static void mdlInitializeSampleTimes(SimStruct *sim_struct) {
    ssSetSampleTime(sim_struct, 0, ${sample-time});
    ssSetOffsetTime(sim_struct, 0, ${sample-offset});
}
/* }}} */
