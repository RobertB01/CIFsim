...
/** Range compatibility tests, for integer type ranges. */
public class RangeCompatIntTest {
    @Test
    @SuppressWarnings("javadoc")
    public void testEqual() {
        RangeCompatTestImpl testImpl = new RangeCompatTestImpl() {
            @Override
            public boolean checkTypeCompat(IntType type1, IntType type2) {
                for (int i = -7; i <= 7; i++) {
            ...
            for (int l2 = -5; l2 <= 5; l2++) {
                for (int u1 = -5; u1 <= 5; u1++) {
                    for (int u2 = -5; u2 <= 5; u2++) {
                        ...
                        // Construct integer types.
                        IntType type1 = newIntType();
                        IntType type2 = newIntType();
                        type1.setLower(l1);
                        type2.setLower(l2);
                        type1.setUpper(u1);
                        type2.setUpper(u2);
