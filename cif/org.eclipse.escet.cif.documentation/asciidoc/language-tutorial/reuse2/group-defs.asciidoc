...
Component parameters can be used inside component definitions to be supplied as parameters to other components.
...
group def ActuatorSet(Sensor sensor):
  actuator1: Actuator(sensor);
  actuator2: Actuator(sensor);
  actuator3: Actuator(sensor);
end

actuatorSet1: ActuatorSet(sensor1);
actuatorSet2: ActuatorSet(sensor2);
----

Group definition `ActuatorSet` models a collection of three actuators.
...
For the actuators part of `actuatorSet1` the provided sensor is `sensor1` and for the actuators part of `actuatorSet2` that is `sensor2`.
