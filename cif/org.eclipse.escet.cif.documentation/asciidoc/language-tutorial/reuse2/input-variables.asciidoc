...
A CIF specification could model a part of a system, while another part of that same system is modeled in another system or language, such as link:http://www.mathworks.nl/products/simulink/[Simulink].
...
Using input variables to couple CIF specifications is somewhat similar to using <<tut-reuse2-chapter-imports,imports>> to split a CIF specification over multiple files.
