...
Discrete distributions can result in only a limited number of possible values.
The lesson that <<lang-tut-stochastics-chapter-intro,introduced stochastics>> used the discrete link:http://en.wikipedia.org/wiki/Bernoulli_distribution[Bernoulli distribution] to model link:http://en.wikipedia.org/wiki/Coin_toss[tossing of a coin], which can result in either heads or tails.
