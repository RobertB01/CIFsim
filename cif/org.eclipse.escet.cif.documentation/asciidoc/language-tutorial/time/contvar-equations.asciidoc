...
In the example above, the derivative of the continuous variable is defined with the declaration, as was already explained in the lesson that introduced <<tut-time-chapter-continuous-variables,continuous variables>>.
...
As with <<tut-reuse1-chapter-algvar-equations,algebraic variables>>, every continuous variable must have a unique derivative in every situation.
