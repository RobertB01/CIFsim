...
* <<lang-tut-functions-statements-switch>>
...
This `signum` function takes a real number `x` and returns its link:https://en.wikipedia.org/wiki/Signum_function[sign].
The `if` statement is used to detect the different situations, and to return the correct result in each of those situations.

indexterm:[function statements,switch]
...
[source, cif]
----
enum mode = OFF, ON1, ON2, ON3, BROKEN;
func int convert_to_speed(mode m):
  return switch m:
    case ON1: 5
    case ON2: 10
    case ON3: 15
...
This `convert_to_speed` function takes an operation mode `m` and returns a speed.
If the value of `m` matches one of the cases, the `switch` statement will evaluate to the indicated value.
...
For example, if `m = BROKEN`, this `switch` statement evaluates to `0`.

One can use all the types from <<lang-tut-values-chapter-overview>>, including user-defined types, as long as the type of the cases matches the switch expression.
In the example, `m` is of type `mode`, which is an enumeration, hence all cases should be of type `mode` as well.

One can also use an automaton as a type of the `switch` statement, where the cases have to be locations of that automaton (for clarity, we omitted the events and the control switch conditions in this example):
...
In the example above, the three cases cover all possible locations of automaton `A`.

`if` statements and `switch` statements are very similar.
It is possible to rewrite a `switch` statement into an `if` statement, see <<tools-cif2cif-chapter-switches-to-ifs>> for the CIF to CIF transformation tool to do this automatically.
