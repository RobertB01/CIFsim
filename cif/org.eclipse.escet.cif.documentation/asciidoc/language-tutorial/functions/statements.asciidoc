...
[source, cif]
----
enum modes = OFF, ON1, ON2, ON3, BROKEN;
func int speed(modes m):
  return switch m:
    case ON1: 5
    case ON2: 10
    case ON3: 15
...
This `speed` function takes an operation mode `m` and returns a speed.
...
In the example, `m` is of type `modes`, which is an enumeration, hence all cases should be of type `modes` as well.
...
`if` statements and `switch` statements have the same functionality.
Therefore, a `switch` statement can be rewritten into an `if` expression, see <<tools-cif2cif-chapter-switches-to-ifs>> for the CIF to CIF transformation tool to do this automatically.
