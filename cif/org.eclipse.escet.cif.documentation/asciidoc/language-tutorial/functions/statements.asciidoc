...
In <<lang-tut-values-chapter-if-expression>> the `if` expression is introduced to construct more complex expressions.
For functions one can also write down `if` statements.
Consider the `if` expression from <<lang-tut-values-chapter-if-expression>>, but now wrapped inside a function:
