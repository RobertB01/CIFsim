...
Information about <<lang-tut-basics-chapter-synchronizing-events-statespace,states>> can be added to locations of automata.
States consist of the current locations of automata and the current values of variables.
For instance, the <<tools-chapter-cif-explorer,CIF explorer>> may generate the following for the example from the lesson on <<lang-tut-basics-chapter-synchronizing-events,synchronizing events>>.
...
For intsance, the first state is represented by location `loc1`, which corresponds to the state where the `consumer` is `idle` and the `producer` is `producing`.
