...
Tools that take CIF specifications as input can processed the annotations that are attached to elements of the specification.
This contrasts annotations from comments, which do not have these properties.
...
* Annotations allow to test out language extensions before actually adding them to the language.
