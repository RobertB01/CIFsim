...
A state annotation adds <<lang-tut-basics-chapter-synchronizing-events-statespace,state>> information to a location in an automaton that represents a state space of another model.
...
The typical use of state annotations is for generated models that contain an automaton representing a state space, where each location of that automaton represents a state of the state space.
