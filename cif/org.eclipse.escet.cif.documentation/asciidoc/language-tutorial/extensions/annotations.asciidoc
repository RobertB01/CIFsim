...
Currently, annotations are only supported for algebraic variables, constants, continuous variables, discrete variables, enumeration literals, enumerations, events, input variables, locations of automata, specifications, and type declarations, but this will be extended to other elements of specifications in the future.
