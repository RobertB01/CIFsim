...
Each type description in this part of the tutorial contains examples of expressions one can make with variables and literals of that type.
The more complex `if` and `switch` expressions are explained in a <<lang-tut-values-chapter-if-and-switch, later lesson>>.
