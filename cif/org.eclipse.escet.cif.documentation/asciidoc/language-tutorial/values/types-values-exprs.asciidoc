...
Each type description in this lesson contains examples of expressions one can make with variables and literals of that type.
The more complex `if` and `switch` expressions are explained at the end of the lesson in <<lang-tut-values-chapter-if-and-switch>>.
