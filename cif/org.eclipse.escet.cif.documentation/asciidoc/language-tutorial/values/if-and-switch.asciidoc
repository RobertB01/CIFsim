...
The `if` and `elif` guard predicates are processed in order.
So once one guard evaluates to true, the 'then' of that expression is evaluated and the remainder of the `elif` expressions and the `else` are skipped.
