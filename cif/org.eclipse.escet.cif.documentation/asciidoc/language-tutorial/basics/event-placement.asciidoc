...
Consider the following CIF specification (the producer/consumer example from the lesson on <<tut-basics-chapter-synchronizing-events,synchronizing events>>):
...
The place where an event is declared is of no influence to the implicit (default) <<tut-basics-chapter-alphabet,alphabets>> of the automata.
