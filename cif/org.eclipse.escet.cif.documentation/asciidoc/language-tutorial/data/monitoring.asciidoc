...
In its `a` location, it disables the `+provide_a+` event, as there is no edge for that event, and the automaton has that event in its (implicit) <<tut-basics-chapter-alphabet,alphabet>>.
