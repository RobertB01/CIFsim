...
The lesson that <<tut-basics-chapter-automata,introduces automata>>, used an example of a lamp:
...
The <<tut-values-chapter-types-values-exprs,data type>> of the variable is `bool`, which means that the variable can only have one of two possible values: `true` or `false`.
...
Furthermore, an other future lesson explains how to <<tut-data-chapter-locs-as-var,use a location as a variable>>.
