...
The lesson on <<tut-data-chapter-discrete-variables,discrete variables>> used the following CIF specification:
...
They may however also be specified outside of the automata, similar to <<tut-data-chapter-init-preds,initialization predicates>>.
