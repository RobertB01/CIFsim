...
Notice that the `producer1` and `producer2` automata and the `consumer1` and `consumer2` automate are identical.
In the lesson on <<lang-tut-reuse2-chapter-aut-def-inst,Automaton definition/instantiation>>, it is shown how this model can be improved.
