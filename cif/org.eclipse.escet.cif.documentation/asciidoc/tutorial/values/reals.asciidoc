...
`1.56` and `-2.7e6` (scientific notation for 2.7 million). Real numbers
must either have a fraction or use the scientific notation, to distinguish them
from integer numbers. The default value of reals (`real` type) is `0.0`.
Several standard arithmetic operators and functions are available to work with
reals, including the following:
...
Integer numbers can often be written where real numbers are expected. Real
