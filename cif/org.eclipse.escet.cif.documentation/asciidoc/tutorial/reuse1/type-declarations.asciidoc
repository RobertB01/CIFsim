...
indexterm:[type,declaration]
Consider this slightly modified version of an example from the lesson on <<tut-reuse1-chapter-constants,constants>>:
...
That is, in both specification the value of constant `M1_DURATION` is a <<tut-values-chapter-dictionaries,dictionary>> with three key/value pairs.

Type declarations can be used to give a type a name, similar to how <<tut-reuse1-chapter-constants,constants>> can be used to give fixed values a name, and <<tut-reuse1-chapter-algebraic-variables,algebraic variables>> can be used to give computations a name.
