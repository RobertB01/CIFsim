...
Algebraic variables can be used to give an <<tut-values-chapter-types-values-exprs,expression>> (computations) a name, similar to how <<tut-reuse1-chapter-constants,constants>> can be used to give fixed values a name.
