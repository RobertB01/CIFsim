...
To get the production duration of products of type `C`, <<tut-values-chapter-types-values-exprs,expression>> `DURATION[C]` can be used.
For more information, see the lessons on <<tut-values-chapter-enumerations,enumerations>> and <<tut-values-chapter-dictionaries,dictionaries>>.
