...
`void` type. Dataless channels are also called `void` channels. When using
dataless channels, the sender does not provide a value to send over the
channel. Also, the receiver cannot use the received value (variable `?`)
since no data is communicated.

Dataless channels are used in the same situations as 'normal' channels,
...
Consider again the producer/consumer example from the
<<tut-channels-chapter-intro,previous lesson>>, with one producer and two
consumers, but without identification numbers for the products:
...
The `producer` still produces products and provides them to either the first
or the second consumer.
