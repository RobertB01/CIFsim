...
If we used only one event, both consumers would need to use that event, have the event in their <<tut-basics-chapter-alphabet,alphabet>>, and would thus have to simultaneously participate in the <<tut-basics-chapter-synchronizing-events,synchronization>>.
...
Channels are declared similar to events, but have a <<tut-values-chapter-types-values-exprs,data type>> that indicates the type of <<tut-values-chapter-types-values-exprs,values>> that are communicated over the channel.
In this case <<tut-values-chapter-integers,integers>> are communicated.
