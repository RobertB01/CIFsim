...
The lesson on <<tut-basics-chapter-synchronizing-events,synchronizing events>> described how events that are used in multiple automata exhibit synchronizing behavior.
...
For instance, consider the following CIF specification (the producer/consumer example from the lesson on <<tut-basics-chapter-synchronizing-events,synchronizing events>>):
