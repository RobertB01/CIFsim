...
The most commonly used <<tut-functions-chapter-intro,kind>> of user-defined functions are the internal user-defined functions.
...
In this example, the function takes a <<tut-values-chapter-lists,list>> of <<tut-values-chapter-reals,real>> values as its only parameter.
Parameter name `vs` can be used in the body of the function to refer to this input value.
Just after the `func` keyword, the <<tut-values-chapter-types-values-exprs,type>> of the _result_ of the computation is specified.
In this case, the function results in a <<tut-values-chapter-reals,real>> typed value.
...
Local variables of functions are very similar to <<tut-data-chapter-discrete-variables,discrete variables>>.
...
The details of the statements are discussed in the <<tut-functions-chapter-statements,next lesson>>.
