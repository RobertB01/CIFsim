...
Assignments in functions are very similar to assignments to <<tut-data-chapter-discrete-variables,discrete variables>>.
For instance:
...
This `signum` function takes a real number `x` and returns its link:http://en.wikipedia.org/wiki/Signum_function[sign].
...
Consider again the `return` statement from the `mean` function from the <<tut-functions-chapter-internal-functions,previous lesson>>:
...
Consider again the `mean` function from the <<tut-functions-chapter-internal-functions,previous lesson>>:
