...
built-in functions, called _standard library functions_. An example is the
`size` function, which takes a <<tut-values-chapter-lists,list>> and
returns the number of elements of the list. Another example is the `abs`
function, which takes an <<tut-values-chapter-integers,integer>> or
<<tut-values-chapter-reals,real>> number and computes the absolute value of
that number.
...
functions allow for more complex computations than would otherwise be possible.
...
information on external user-defined functions.
