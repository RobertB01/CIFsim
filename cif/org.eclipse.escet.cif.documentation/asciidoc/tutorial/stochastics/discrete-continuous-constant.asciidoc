...
CIF features over a dozen different stochastic distributions. These
...
The lesson that <<tut-stochastics-chapter-intro,introduced stochastics>>
used the discrete
link:http://en.wikipedia.org/wiki/Bernoulli_distribution[Bernoulli distribution]
to model
link:http://en.wikipedia.org/wiki/Coin_toss[tossing of a coin], which can result
in either heads or tails.

Throwing a
link:http://en.wikipedia.org/wiki/Dice[dice]
can result in six
...
link:http://en.wikipedia.org/wiki/Uniform_distribution_(discrete)[discrete uniform distribution]:
...
The `uniform` function is used to create a uniform integer distribution that
produces <<tut-values-chapter-integers,integer>> samples from the range
[1..7) (inclusive on the left side, exclusive on the right side). Each sample
is stored in `result` and added to the `results`. If we were to simulate
this model forever, the `results` list would contain equal amounts of each of
the six values.
...
Continuous distributions return a value from a continuous range. An example is
the weight of
link:http://en.wikipedia.org/wiki/Penny_(United_States_coin)[pennies].
This
...
with a
link:http://en.wikipedia.org/wiki/Mean[mean]
of 2.46 grams and a
link:http://en.wikipedia.org/wiki/Variance[variance]
of 0.0004 grams:
...
The `normal` function is used to create a normal distribution with a mean of
`2.46` and a variance of `0.0004`. The sampled weight is stored in the
`weight` variable each time the distribution is sampled. The probability of
the different weights as a result of the used normal distribution is depicted
in the following figure:

image::{tut-imgsdir}/stochastics/pennies.png[]

indexterm:[stochastics,constant]

=== Constant distributions

When developing a model with stochastic behavior, it can be hard to validate
...
Constant distributions always produce the same sampled value. Consider the
...
This model is identical to the dice model one used earlier in this lesson,
except for distribution variable `d`, which now holds a constant distribution
that only produces value `3` when sampled.
