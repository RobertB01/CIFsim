...
Discrete distributions can result in only a limited number of possible values.
The lesson that <<tut-stochastics-chapter-intro,introduced stochastics>> used the discrete link:http://en.wikipedia.org/wiki/Bernoulli_distribution[Bernoulli distribution] to model link:http://en.wikipedia.org/wiki/Coin_toss[tossing of a coin], which can result in either heads or tails.

Throwing a link:http://en.wikipedia.org/wiki/Dice[dice] can result in six possible outcomes: 1, 2, 3, 4, 5, and 6.
For a fair dice, each of the outcomes is equally likely, with chance 1/6.
This can be modeled using a link:http://en.wikipedia.org/wiki/Uniform_distribution_(discrete)[discrete uniform distribution]:
...
The `uniform` function is used to create a uniform integer distribution that produces <<tut-values-chapter-integers,integer>> samples from the range [1..7) (inclusive on the left side, exclusive on the right side).
...
An example is the weight of link:http://en.wikipedia.org/wiki/Penny_(United_States_coin)[pennies].
This could for instance be approximated using a continuous link:http://en.wikipedia.org/wiki/Normal_distribution[normal distribution] with a link:http://en.wikipedia.org/wiki/Mean[mean] of 2.46 grams and a link:http://en.wikipedia.org/wiki/Variance[variance] of 0.0004 grams:
