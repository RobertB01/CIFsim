...
An example is a link:http://en.wikipedia.org/wiki/Coin_toss[coin toss], where the result can be either heads or tails.
Tossing a coin exhibits link:http://en.wikipedia.org/wiki/Randomness[randomness].
It is possible to model the different variants in a CIF model without specifying the likelihood of each of the outcomes.
This approach was used in the lesson on <<tut-basics-chapter-non-determinism,non-determinism>>.

indexterm:[stochastics,distribution]
It is however also possible to explicitly specify the likelihood of each of the outcomes in the CIF model, using a _stochastic distribution_ (also called link:http://en.wikipedia.org/wiki/Probability_distribution[probability distribution]).
...
The link:http://en.wikipedia.org/wiki/Bernoulli_distribution[Bernoulli distribution] for instance, can be used to model things with two potential outcomes, such as a coin toss.
...
Variable `d` holds a stochastic distribution that produces <<tut-values-chapter-booleans,boolean values>> (`true` or `false`), as indicated by its `dist bool` <<tut-values-chapter-types-values-exprs,type>>.
...
This is further explained in one of the next lessons, which explains <<tut-stochastics-chapter-pseudo-randomness,pseudo-randomness>>.
