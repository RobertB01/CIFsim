...
It may for instance be modeled as a state machine.
...
Verification involves checking and ultimately ensuring that the controller (model) satisfies its specified requirements.
...
While manual implementation is possible, the code is often automatically generated.

[[sbe-approaches-model-based-benefits]]
=== Benefits of model-based engineering

Model-based engineering has several advantages over <<sbe-approaches-traditional,traditional engineering>>:
...
The use of such formal models leads to unambiguous interpretation of control requirements.
+
...
This allows to cover much more behavioral scenarios, increasing confidence in correctness of the the controller implementation.
+
indexterm:[validation]
indexterm:[simulation]
+
Another example of this is validation of the specification by means of simulation.
Using simulation various execution scenarios can be examined, to give insight into the behavior of the controller.
...
A great benefit of model-based engineering is that this can be done already during the earlier phases of development, rather than only at later phases such as implementation or testing.
...
It is also possible to switch to a different platform at a later time, or additionally generate code for other platforms.

indexterm:[model, single source of truth]

Up-to-date models are the single source of truth::
Model-based engineering thus put models at the center of attention.
...
The models are thus the 'single source of truth'.
...
Properties that a controlled system must satisfy, even if they are not satisfied in the uncontrolled system.
...
Also called a <<sbe-supervisory-controllers,supervisory controller>>, supervisor, or simply controller, in https://en.wikipedia.org/wiki/Control_theory[control theory].
It precisely specifies how the state of the controller changes when a sensor signal changes, and under what conditions and in which states an actuator may be turned on or off.
...
For instance, a model of control requirements in the form of logical formulas, or a model of the controller represented as a state machine.

indexterm:[engineering, model-based]

Model-based development/engineering::
Placing models at the center of the entire development process and the entire lifecycle of the system, including design, implementation and maintenance.
...
Validation::
The process of checking and ultimately ensuring that the controller (or controller model) exhibits the desired behavior, and thus ensuring that the controller is the desired controller.
Since a controller must satisfy its specified requirements, this includes validating the requirements to ensure they are the desired requirements.
