...
The software source code may for instance be written in Java, C or PLC programming languages.

indexterm:[verification]

* **Verification** involves checking the realized controller against its requirement and controller designs, to ensure that the controller is made in the right way.
That is, the controller must be correctly realized as designed.
...
The columns of the table indicate various engineering approaches.
From left to right, they employ progressively more automation and computer assistance.
Typically, the approaches can be characterized as follows:
...
The controller behavior is modeled in a formal way, allowing a computer to interpret and analyze it.
...
From them, all kinds of artifacts can be generated automatically, including the software code of the implementation.
...
Simulation models can be generated to allow simulating the system behavior and validating the requirements during early phases of development.

indexterm:[engineering, verification-based]

* **Verification-based engineering** automates the verification step.
It uses formal verification, a mathematical technique that can automatically check the controller model against its requirements.
It either indicates that the specified requirements are satisfied by the controller model, or it provides counter examples that indicate in which situations they are not satisfied.
Through formal verification, the controller model can be iteratively adapted to satisfy all specified requirements.
...
* **Synthesis-based engineering** automates the design of the controller.
It uses supervisory controller synthesis to automatically synthesized a controller model from requirements and simple models of the uncontrolled system.
