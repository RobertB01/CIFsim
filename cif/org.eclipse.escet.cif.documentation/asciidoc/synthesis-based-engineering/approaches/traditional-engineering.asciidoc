...
This may for instance be done in-house within the company, by different teams or departments.
However, the documents can also serve as input and as a contract to a supplier to develop the control software.

indexterm:[verification]
indexterm:[validation]

Finally, the implementation is verified and validated.
...
Ensuring that the textual descriptions of all these cases do not leads to inconsistencies is often practically undoable.
+
A good domain expert will be able to limit the number of mistakes, such as missing requirements and contractions in the requirements specification, but typically can't completely eliminate them.
