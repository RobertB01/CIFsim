...
The specified requirements could not be the desired requirements, as they could for instance be wrongly specified or too strict, resulting in the controller exhibiting unwanted or insufficient behavior.
...
The plant model clearly states that it is not possible for the gate to be closed and open at the same time, as the plant model can only be in one state at a time.
...
* **Non-blocking:** It doesn't block, i.e., a marked state can always be reached from every reachable state, thus ensuring a form of link:https://en.wikipedia.org/wiki/Liveness[liveness].
