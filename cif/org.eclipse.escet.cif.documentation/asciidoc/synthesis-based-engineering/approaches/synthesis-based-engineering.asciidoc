...
And the controller model must still be validated to ensure it behaves as intended.
While synthesis guarantees that the controller correctly adheres to the specified requirements, the specified requirements could not be the desired requirements.
For instance, the requirements could be wrongly specified or too strict, resulting in the system being controlled by the controller exhibiting unwanted or insufficient behavior.
