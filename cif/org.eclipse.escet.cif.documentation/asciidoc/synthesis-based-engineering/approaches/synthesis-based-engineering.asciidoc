...
* **Non-blocking:** It doesn't block, i.e., a <<sbe-tips-tricks-marking,marked state>> can always be reached from every reachable state, thus ensuring a form of link:https://en.wikipedia.org/wiki/Liveness[liveness].
