...
If the operation has failed, the system must _redo_ the operation on that product.
...
* Successfully processed products must exit the operation area (_op_exit_).
...
And a product is at the _CHK_ place and the check indicates the product was successfully processed.
...
[upperalpha]
. If a product is checked and found to be successfully processed, it can only exit (_op_exit_).
...
There may be two products in the processing loop if one of them is a successfully processed product about to exit the processing loop.
