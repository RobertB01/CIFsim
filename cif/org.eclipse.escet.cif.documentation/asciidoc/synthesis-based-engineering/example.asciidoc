...
Products _enter_ at the _IO_ place.
From there production can _start_ by moving them to the dispatcher (_DISP_).
The dispatchers dispatches a product (_do1_ or _do2_) to one of two operators (_OP1_ or _OP2_) that perform the same operation.
Once the operation is completed (_done1_ or _done2_), the product is moved to the checker (_CHK_).
The product is either considered OK or not OK (NOK).
If it is not OK, it must _redo_ the operation.
This may be repeated until the operation is successful.
The product then moves to _DONE_ (via _op_exit_) and back to _IO_ (via _finished_).
There it may _exit_ the system.

The gray boxes indicate places where at most one product can be located at a time.
The moving of products through the system is visualized by the labeled black arrows in the figure.
...
A sensor indicates the result of the check performed on processed products, indicating whether they are OK or not OK.
This sensor is under the control of the system, not the controller.

=== Plants and requirements

The uncontrolled system (the plant) already ensures that:

* Products that _enter_ can only _start_, preventing them from exiting without having been processed.
* Products that are not OK must _redo_ the operation.
* Products that are OK must exit the operation area (_op_exit_).
* Once a product is _finished_ it must _exit_, preventing it from being processed again.
...
The controller must restrict the behavior of the uncontrolled system such that it satisfies the requirement.
It can only do so by controlling the movement of products through the system.

The requirement is specified in natural language as a short and simple sentence.
It can similarly be quite easily modeled.
...
When a product is about to exit the system, it is in the _IO_ place.
If the identifier of the current product on the _IO_ place is given by _curId_, then the requirement can be formulated as:

* _curId_ = _lastExitId_ + 1

See the section on <<sbe-with-cif,synthesis-based engineering with CIF>> for how the plants and requirement of this example can be modeled in CIF.

=== Synthesis-based engineering

There are various ways to ensure the requirement holds.
For instance, a controller could disallow products to enter the system altogether.
But that would not result in a useful controlled system.
Another option is to only allow a single product to be processed at a time.
...
** At the dispatcher place, two operator places, and checker place, there is in total at most one product.
** At the dispatcher place and two operator places there is in total at most one product.
And a product is at the checker and the check indicates the product is OK.

. Movement _done1_ is only allowed if the following two conditions both hold:
** Either there is no product at the dispatcher, or it is a later product than at the first operator.
** Either there is no product at the second operator, or it is a later product than at the first operator.

. Similarly, movement _done2_ is only allowed if the following two conditions both hold:
** Either there is no product at the dispatcher, or it is a later product than at the second operator.
** Either there is no product at the first operator, or it is a later product than at the second operator.
...
[upperalpha]
. If a product is checked and found to be OK, it can only exit (_op_exit_).
...
This way, if a product is not OK, it can be redone over and over again, while the other product is at one of the operators.
With three or more products in the processing loop, this is not possible.
An exception to 'at most two products in the processing loop' rule is when a product has been checked and found to be OK.
Then, a third product may be present, as the OK product can then leave the processing loop and at most two products will remain in the processing loop.
...
There may be two products in the processing loop if one of them is an OK product about to exit the processing loop.
...
Then only at most one of the _IO_ and _DONE_ places may contain a product, for a total of four products in the system.
To understand why this is the case, consider the following:

** A product could be at the _IO_ place.
But then no product must be at the _DONE_ place.
If there were a product at the _DONE_ place, there would be products at the _IO_, _DONE_ and _CHK_ places.
The product at the _CHK_ place could then not move to the _DONE_ place, as that already has a product.
Similarly, the product at the _DONE_ place could then also not move to the already occupied _IO_ place.
And the product at the _IO_ place could then not move to the _DISP_ place, as the processing loop is already maximally filled.
...
But then, by similar reasoning, no product must be at the _IO_ place.
...
If you consider that this is only a simple example system, with only a single requirement, it is clear that having some computer assistance when engineering a controller can be very useful.
...
* Synthesis automatically computes the optimal control conditions.
...
* As an alternative to synthesis, we could apply <<sbe-approaches-verification-based,formal verification>> on the uncontrolled system and requirement.
However, we would get only a counter example representing a scenario where the requirement does not hold.
...
The control conditions produced by synthesis however, are *minimally restrictive*.
...
The various parts of the system, as well as the requirement, <<sbe-with-cif,can be *modeled>> separately*.
This makes it easy to adapt the plant, to for instance allow an already OK product to bypass the processing loop.
With minimal changes to the plant, and no changes to the requirement, a new supervisor can then be produced by the push of a button.
