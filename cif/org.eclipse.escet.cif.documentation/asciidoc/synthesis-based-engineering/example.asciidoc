...
This sensor works autonomously, it is not controlled.
...
See the section on <<sbe-with-cif,synthesis-based engineering with CIF>> for how the example system and its requirement can be modeled in CIF.
...
And a product is at the _CHK_ place and the check indicates the product is OK.
...
This makes it easy to adapt the system (model), to for instance allow an already OK product to bypass the processing loop.
