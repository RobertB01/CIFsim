...
This may reveal additional issues, for instance when the behavior of the system is not properly captured in the <<sbe-in-practice-steps-modeling-the-plant,plant model>>, or when the inputs and outputs of the system are not properly connected to the controller.
