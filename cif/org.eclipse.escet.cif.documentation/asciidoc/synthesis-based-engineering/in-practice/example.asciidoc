...
By using <<tools-cifsim-input-svg-chapter-svgin,CIF/SVG input mappings>>, clicks on elements in SVG image trigger events in the model.
Here are some examples:
...
The labeled edges in the figure can be clicked to trigger the corresponding event.
For example, clicking the edge labeled with `enter` triggers event `c_enter`.
When an edge is clicked and the corresponding event is not enabled, a warning will be printed to the console.
