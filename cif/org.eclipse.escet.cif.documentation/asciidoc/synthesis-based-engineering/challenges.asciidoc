...
For instance, use <<sbe-approaches-verification-based,formal verification>> to guarantee that all specified requirements are satisfied.
...
Use supervisory controller synthesis, and fully adopt <<sbe-approaches-verification-based,verification-based>>.
...
The selected tools should also be used in the right way.
...
Employing the <<sbe-tips-tricks-non-monolithic-synthesis,proper techniques>> for the given situation is essential to mitigate such concerns.
