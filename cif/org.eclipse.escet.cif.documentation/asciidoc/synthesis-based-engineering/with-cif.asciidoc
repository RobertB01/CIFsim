...
* Variable `curId` represents the product id of the product that is currently located at the _IO_ place.
However, its value is irrelevant when there is no product at the _IO_ place.
...
<<lang-tut-basics-automata-loc-init,Initially>> the _IO_ place is `Idle`, as it has no product.

A product may only enter (by event `c_enter`) if the maximum number of products is not yet exceeded (`cnt < MAX_NR_OF_PRODS`).
If a product enters, a product is present at the _IO_ location, and it moves to its `NewProduct` location (`goto NewProduct`).
As a result of this, it also gets assigned the next product id (`curId := nextId`), then being the product currently located at the _IO_ place.
...
After moving the product to the dispatcher, the _IO_ place no longer holds a product (`goto Idle`).
...
While `Idle` the _IO_ place may receive a _finished_ product (`c_finished?`) from the _DONE_ place.
...
There is then no longer a product at the _IO_ place.
...
This specifies that the _IO_ place  must always be able to become idle again.
Through similar markings in the other places also the system as a whole must always be able to become idle again.

==== The other places

For brevity, we'll not explain the remaining places in as much detail as the _IO_ place.
We'll discuss each automaton for each place briefly:

* The dispatcher (_DISP_) receives products (`c_start?`) from the _IO_ place.
...
* The _DONE_ place simplify forwards products from the checker (_CHK_) to the _IO_ place.
...
[source, cif]
----
requirement IO.FinishedProduct => IO.curId = ((IO.lastExitId + 1) mod MAX_NR_OF_PRODS);
----

If the _IO_ place has a finished product (is in its `FinishedProduct` location), then a product is about to exit the system.
We know the product id of the last product that exited (`IO.lastExitId`) and the product id of the product currently situated at the _IO_ place (`IO.curId`).
...
svgout id "io"              attr "fill"       value if IO.Idle:  COLOR_IDLE else COLOR_BUSY       end;
...
An occupied place is light blue, with its smaller indicating the product id of the product that is present.
...
Finally, it indicates the product id of the last product that existed, as indicated by `IO.lastExitId`, or `n/a` if no product has exited thus far.
