...
The idea of an interchange format (CIF) was born to reduce the number of transformations that are needed.
...
By doing such transformations on the interchange format itself, by using CIF to CIF transformations to rewrite CIF models, they could be reused, rather than having to implement them again for each transformation.
To support this, the semantics of CIF needed to be compositional.
With a compositional semantics, parts of the model could be replaced without changing the meaning of the model.
