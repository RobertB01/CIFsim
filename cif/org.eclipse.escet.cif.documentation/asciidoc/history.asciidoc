...
Languages and tools considered to be in scope at the time, were those from computer science and from dynamics and control, for modeling, simulation, analysis, controller synthesis, and verification in the area of discrete event, timed, and hybrid systems.
...
The idea of an interchange format was born, to reduce the number of transformations that are needed.
Instead of transforming _n_ languages each to _n - 1_ others, each language would only need to be transformed to and from an interchange format.
For instance, a UPPAAL model could be transformed to the interchange format, and then to a Modelica model.
That way, only _2 * n_ transformations would be needed, reducing the effort to implement the transformations.

The interchange format also needed to be rich enough to represent the concepts from the various languages.
Only then could a large enough subset of the language features of the various languages be transformed.
Hence, it needed concepts from discrete event, timed, and hybrid systems.
...
To enable this, it was deemed necessary for the interchange format to have a formal semantics, grounded in mathematics, and independent of any implementation aspects.

As different languages may have different concepts, part of the transformation could be to transform the unsupported concepts to other equivalent concepts, that are supported.
By doing such transformations on the interchange format itself, they can be reused, rather than having to implement them again for each transformation.
To support this, the semantics of the interchange format should be compositional.
With a compositional semantics, parts of the model can be replaced, without changing the meaning of the model.
...
After a few years, several issues were identified with CIF 1.
...
To solve the difficulties with CIF 1, it was decided to start from scratch, with CIF 2.
...
And the implementation technology around metamodels and model transformation languages was not very mature at the time.
...
The SEAL language was considered a nice modeling language, but was limited to supervisory controllers, and lacked concepts to express timed and hybrid systems.
Introducing another language, besides CIF 2, also created confusion in collaborations with other parties.

=== CIF 3

Not being satisfied with CIF 2, the idea came to develop CIF 3.
With the SEAL language being considered a have a good modeling syntax, it was taken as the basis for CIF 3.
However, having two languages, CIF 3 and SEAL, with considerable overlap, required quite a significant development and maintenance effort.
...
However, even with these considerable efforts, CIF 3 remained an academic tool, preventing wide-spread adoption of CIF in industry.
