...
Transformations could be implemented using link:https://en.wikipedia.org/wiki/Model_transformation_language[model transformation languages] such as ATL and QVTo, rather than general purpose languages such as Java.
...
SEAL was an untimed, discrete-event only, automata-based language for the specification of controllers.
...
With the SEAL language being considered to have a good modeling syntax, it was taken as the starting point for CIF 3.
As a consequence of that, CIF 3 and SEAL had considerable overlap.
Developing these two closely related languages required quite a significant additional development and maintenance effort.
Therefore, rather than having two subtly different languages, the goals of CIF 3 were extended to also cover the goals of SEAL.
CIF 3 replaced SEAL and the development of SEAL stopped.
