...
Annotation argument names are now optional, although for specific annotations they may still be mandatory.
...
See the documentation about documentation annotations for further details.
Annotations are still an experimental work-in-progress language feature.
Their design may change in a backward incompatible manner (issue {escet-issue}801[#801]).
...
Annotations on specifications and on invariants in locations require double `@`-signs, such as `@@doc` instead of `@doc`.
Annotations are still an experimental work-in-progress language feature.
...
A constant should be used instead (issue {escet-issue}797[#797]).
