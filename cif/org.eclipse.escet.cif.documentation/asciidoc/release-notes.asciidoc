...
* CIF PLC code generator now supports S7 output for SIMATIC controllers (issue #142).

Improvements and fixes:

* CIF type checker no longer produces the wrong warning for globally disabled event that are monitored, for events referred to via component parameters or component instantiations (issue #300).
* CIF type checker no longer produces false positive/negative warnings and errors related to the use of events in component definitions/instantiations.
...
It also has several other small improvements related to these checks, including improved warning messages and reporting the warnings at better positions (issues #298 and #301).
* CIF simulator interactive GUI input component now has a horizontal scrollbar from the start, if applicable (issue #217).
* CIF simulator termination by the red stop button no longer leaves the interactive GUI input component buttons enabled (issue #274).
* CIF PLC code generator PLC number bits option now has a new automatic mode.
The default remains 64-bit for PLCopen XML, IEC 61131-3 and TwinCAT output (issue #142).
* CIF PLC code generator now generates correct assignments to temporary variables for multi-assignments (issue #282).
* CIF PLC code generator now generates calls to the `NOT` function without formal argument names (issue #286).
