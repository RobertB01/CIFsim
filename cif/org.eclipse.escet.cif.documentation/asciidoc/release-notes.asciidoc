...
* The deprecated curly-brackets syntax for declaring enumerations has been removed.
It has been deprecated for years, and deprecation warnings were present since release v0.2.
...
* The CIF PLC code generator's deprecated _Eliminate enumerations_ option has been removed.
It was deprecated since release v0.2.
Use the _Convert enumerations_ option instead, which allows for more control (issue {escet-issue}613[#613]).
* The deprecated CIF to CIF transformation to eliminate enumerations (`elim-enums`) has been removed.
It was deprecated since release v0.2.
Use the transformation to convert enumerations to integers instead, as it has the same functionality (issue {escet-issue}613[#613]).
...
It should be considered experimental, and is disabled by default.
...
* The position information for invariants has been improved.
This affects various CIF tools.
For instance, it improves with invariants as reported by the CIF type checker.
It also improves reporting of precondition violations by several CIF tools, including the CIF PLC code generator, the CIF to Supremica transformation, and the CIF explorer (issue {escet-issue}591[#591]).
