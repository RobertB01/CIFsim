...
By default, still the non-blocking states are computed first, then the controllable states, and finally and reachable states (if enabled).
Using the option, the order can be changed.
...
Instead, this information is now one of the statistics that can be enabled using the _Statistics_ options.
The new statistic is disabled by default.
If enabled, the statistic is printed to normal output.
...
It now also has greater precision (issue {escet-issue}610[#610]).
* The CIF data-based synthesis tool's legacy hyper-edge creation algorithm now properly considers invariants in locations for variable ordering, rather than considering invariants in the automaton twice.
This could affect synthesis performance (issue {escet-issue}377[#377]).
* The CIF data-based synthesis benchmarking scripts no longer produce debug output when performing synthesis, making benchmarking faster and producing less output, while still producing the same values for platform-independent metrics and number of states in the controlled system (issues {escet-issue}526[#526] and {escet-issue}564[#564]).
