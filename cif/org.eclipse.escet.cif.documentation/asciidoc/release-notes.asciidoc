...
* Annotations may now be added to automata, automaton definitions, automaton instantiations, enumerations, enumeration literals, events, functions, function parameters, function variables, groups, group definitions, group instantiations, invariants, specifications and type declarations.
Annotations on specifications and on invariants in locations require double `@`-signs, such as `@@doc` instead of `@doc` (issue {escet-issue}593[#593]).
* The CIF type checker now warns about certain declarations that have a type that supports only one value.
This new check is enabled for algebraic parameters, algebraic variables, channel parameters, channels, discrete variables, function parameters, function local variables, function return types and input variables.
...
* The CIF to CIF anonymize names transformation now renames local variables of functions to `fvarNN` rather than to `discNN`, with `NN` a number (issue {escet-issue}811[#811]).
...
* The CIF code generator for Java, JavaScript and HTML now generate improved comments in the generated code for user-defined functions (issue {escet-issue}593[#593]).
* The (still experimental) new CIF PLC code generator now only supports event that are declared as controllable or uncontrollable.
...
* The CIF documentation has various small improvements and fixes (issues {escet-issue}593[#593], {escet-issue}628[#628], {escet-issue}804[#804], {escet-issue}810[#810] and {escet-issue}811[#811]).
