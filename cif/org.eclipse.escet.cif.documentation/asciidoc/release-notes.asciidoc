...
- CIF to CIF 'component definition/instantiation' transformation no longer produces invalid results for models with algebraic parameters with `int` type instantiated with a constant value (issue #105).
...
- CIF data-based synthesis now properly ensures requirement invariants are rebranded as supervisor invariants in the synthesis output (issue #117).
