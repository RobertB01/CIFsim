...
The JavaScript code is part of an HTML page, which allows simulating the model in a browser.
The JavaScript code generation is currently being developed, and should be considered experimental (issue {escet-issue}272[#272]).
...
* The CIF controller checker now produces more debug output (if debug output is enabled), to be able to better track what its progress (issue {escet-issue}686[#686]).
...
* The CIF code generator now generates valid Java and Simulink code for models without enumerations and without any automata that have at least two locations (issue {escet-issue}272[#272]).
