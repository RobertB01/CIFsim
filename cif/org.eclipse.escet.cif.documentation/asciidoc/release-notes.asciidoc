...
- CIF to mCRL2 transformation precondition check messages improved to avoid duplicates for multiple initial locations (issue #60).
- CIF to mCRL2 documentation updated to reflect that input variables are not supported (issue #59).
- CIF to Supremica and CIF to UPPAAL precondition check message wording for discrete variables with multiple potential initial values has been fixed (issue #58).
