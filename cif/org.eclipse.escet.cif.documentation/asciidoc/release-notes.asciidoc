...
This new feature is currently work-in-progress, undocumented, and not yet supported by all CIF tools (issue {escet-issue}227[#227]).
* The syntax for addressables (the left hand side of an assignment) has changed to allow names like `group.variable`, but this is only accepted for SVG input mappings, not for assignments in functions or on edges (issue {escet-issue}227[#227]).
* Annotations may now be added to discrete and continuous variables.
...
* The CIF benchmark models have been extended with the `mri_event` and `mri_state` benchmark models (issue {escet-issue}364[#364]).
...
* The CIF code generator now supports `doc` annotations on discrete variables.
The comments in the generated C, Java, JavaScript, HTML, and Simulink code for `doc`-annotated discrete variables now include the documentation from their documentation annotations (issue {escet-issue}593[#593]).
