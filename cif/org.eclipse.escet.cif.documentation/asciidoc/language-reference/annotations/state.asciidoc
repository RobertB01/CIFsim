...
A state <<lang-ref-extensions-annotations,annotation>> adds <<lang-tut-basics-chapter-synchronizing-events-statespace,state>> information to a location in an automaton that represents a state space of another model.
...
* Each argument represents an automaton with its current location, or a variable with its current value.
* There can be any number of arguments (including no arguments), as models may have any number of automata and variables.
* The name of the argument should be the absolute name of the corresponding automaton or variable.
* Argument values, or parts of argument values (in case of containers), must be literals of type `bool`, `int`, `real`, `string`, `tuple`, `list`, `set`, or `dict`.
...
| String literal with the name of the location, or `"*"` for a nameless location.
If the name is a keyword, it should not escaped.

| Enumeration literal
| String literal with the non-escaped name of the literal.
If the name is a keyword, it should not escaped.

| Function
| String literal with the non-escaped absolute name of the function.
For functions that represent a default initial value of a variable with a function type, `"*"` should used as function name.
Keywords in the absolute names should not be escaped.
|===
