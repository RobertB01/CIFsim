...
And there is a comma after each argument, including the last argument.
...
+
An annotation provider could for instance limit the elements that may be annotated with the annotation, or require all elements of a certain type to be annotated if at least one of them is annotated.
...
If you add a custom annotation, that is not a built-in annotation delivered along with CIF itself, consider the following:
