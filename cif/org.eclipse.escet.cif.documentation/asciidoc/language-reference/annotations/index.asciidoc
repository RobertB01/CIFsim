...
If specified, annotations are indicated by a <<lang-ref-syntax-lexical-specannoname,specification annotation name>> (for specifications) or a regular <<lang-ref-syntax-lexical-annoname,annotation name>> (for all other elements of specifications).
...
If both a name and a value are provided, they separated by a `:` sign.
...
Similarly, if the argument name is a relative name, and any part of the relative name is a keyword, that part needs to be escaped.
For instance, argument name `automaton` needs to be escaped as `$automaton`, and argument name `some.automaton.or.so` needs to be escaped as `some.$automaton.$or.so`.
...
An annotation may have multiple unnamed arguments.
