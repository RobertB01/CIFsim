...
[[lang-ref-syntax-lexical-annoname]]
`ANNOTATIONNAMETK`::
A (regular) annotation name.
Annotation names are used to refer to annotations.
+
Defined by the regular expression:
`+@[a-zA-Z_][a-zA-Z0-9_]*(:[a-zA-Z_][a-zA-Z0-9_]*)*+`.
...
[[lang-ref-syntax-lexical-specannoname]]
`SPECANNOTATIONNAMETK`::
A specification annotation name.
Specification annotation names are used to refer to annotations, when they are used to annotate specifications.
Specification annotation names are identical to regular annotation names, but starts with two at signs (`@@`).
+
Defined by the regular expression:
`+@@[a-zA-Z_][a-zA-Z0-9_]*(:[a-zA-Z_][a-zA-Z0-9_]*)*+`.
