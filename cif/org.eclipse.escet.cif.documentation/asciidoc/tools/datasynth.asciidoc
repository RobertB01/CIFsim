...
* _Event warning_: Whether to warn for events that are never enabled in the input specification or always disabled by the supervisor.
...
If there are controllable events that are never allowed by the supervisor, a warning is printed to the console.
