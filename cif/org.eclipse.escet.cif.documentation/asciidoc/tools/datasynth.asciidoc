...
* _Event order_: Edges are 'applied' to update the various BDDs during calculation.
The order of the applied edges is determined by this option.
For more information, see the <<tools-datasynth-event-order>> section below.
...
indexterm:[data-based supervisory controller synthesis,event order]

[[tools-datasynth-event-order]]
=== Event order

TODO: General information about edge/event ordering.

The order in which the edges are applied is determined by the _Event order_ option (see <<tools-datasynth-options,options>> section above).
Several default orders exist, and it is also possible to define a custom order.
By default, the model order is used.
The following default orders exist:

* _model ordering_ (option value `model`)
+
The order of the edges is as they occur in the model.
Edges for input variables are always put after the other edges, sorted based on the variable order.

* _reverse model ordering_ (option value `reverse-model`)
+
The order of the edges is as they occur in the model, but reversed.
Edges for input variable are always put before the other edges, sorted based on the reversed variable order.

* _sorted ordering_ (option value `sorted`)
+
The order of the edges is based on the names of the events and input variables.
They are sorted alphabetically in ascending order, based on their absolute names.
In case edges are labeled with the same event, the edges are ordered based on the model order.
...
In case edges are labeled with the same event, the edges are ordered based on the reversed model order.
...
In case of multiple matches, the matches are sorted increasingly on their absolute names.

Multiple names can be separated with `,` characters.
The edges matching the name pattern before the `,` are ordered before the edges matching the name pattern after the `,`.
...
| <<tools-datasynth-event-order>>
| Better order for smaller BDD representations
| Choose the best order, depends on the model, model usually good choice, custom order allows for best performance
