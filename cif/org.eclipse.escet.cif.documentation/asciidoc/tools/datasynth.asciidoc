...
Only once all edges have been applied and none of them leads to new states being found has a fixed point been reached and the computation stops.
...
In case edges are labeled with the same event or input variable, the edges are ordered based on the linearized model order.
...
In case edges are labeled with the same event or input variable, the edges are ordered based on the reversed linearized model order.
