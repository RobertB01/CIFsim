...
* _Requirement reference warning_: Whether to warn for plant automata or plant invariants that reference requirement automata.
Is enabled by default.
Disabling this warning may increase the performance of synthesis.
...
The synthesis algorithm also checks for plant invariants or plant automata that reference requirement automata.
If such a problem is detected, a warning is printed to the console.
Among others, references in guards, updates, invariant predicates, initialization predicates, marked predicates and initial values for discrete variables are detected.
...
| <<tools-detasynth-early-prob-detect>>
| Warning for plants that reference requirements
| Disable for better performance
|===
