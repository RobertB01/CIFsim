...
Different algorithms may use different representations of the variable relations, such as _hyper-edges_ or _graph edges_.
Further information about these representations may be obtained by enabling <<tools-datasynth-dbg-output,debug output>>.
...
It then chooses the best order out of the orders produced by these two algorithms as well as their reverse orders, based on the total span metric.
+
