...
Is printed to the console, after execution, just before termination of the tool.
...
Is printed to the console each time a BDD garbage collection is performed.
...
Is printed to the console each time a BDD node table resize is performed.
See also the BDD library initial node table size option (see <<tools-datasynth-options,options>> section above).

* _Maximum used BDD nodes_ [`bdd-max-nodes`]
+
The larger a BDD is, the more memory is needed to store it. 
The size of a BDD can be expressed as the number of nodes used, which is implementation and machine independent.
This statistic prints the maximum number of BDD nodes used during synthesis.
Is printed to the console, after execution, just before termination of the tool.
Note that collecting this statistic makes synthesis take longer than without collecting it.
This statistic is discussed in more detail in the following paper: Sander Thuijsman, Dennis Hendriks, Rolf Theunissen, Michel Reniers, Ramon Schiffelers, Computational Effort of BDD-Based Supervisor Synthesis of Extended Finite Automata, IEEE Conference on Automation Science and Engineering, Pages 486-493, 2019.

* _Continuous BDD performance statistics_ [`bdd-cont-perf`]
+
The larger a BDD is, the more memory is needed to store it. 
The size of a BDD can be expressed as the number of nodes used, which is implementation and machine independent.
This statistic prints the number of BDD nodes used during synthesis over 'time', where the BDD operations are used to represent time, i.e., each BDD operation is one time unit.
The printed information includes a table where each row contains the number of performed BDD operations from the start of synthesis and the used BDD nodes at that point in time.
Is printed to the _continuous performance statistics file_, after execution, just before termination of the tool (see <<tools-datasynth-options,options>> section above).
Note that collecting these statistics makes synthesis take longer than without collecting it.
This statistic is discussed in more detail in the following paper: Sander Thuijsman, Dennis Hendriks, Rolf Theunissen, Michel Reniers, Ramon Schiffelers, Computational Effort of BDD-Based Supervisor Synthesis of Extended Finite Automata, IEEE Conference on Automation Science and Engineering, Pages 486-493, 2019.

* _BDD cache_ [`bdd-cache`]
+
The BDD library that is used implements a cache to speed up the calculations.
The printed information includes the number of times the cache is accessed, the number of hits, and the number of misses.
Is printed to the console, after execution, just before termination of the tool.
In the table below shows all metrics printed and their meaning.
For more information, see the <<tools-datasynth-op-cache>> section below.
+
...
| Unique access
| The number of times the cache is accesses because a node needed to be created.

| Unique chain
| The number of times the cache needs to check the next node in the cache.

| Average chain
| The relative unique chain size with respect to the unique access size.

| Unique hits
| The number of times the node was already in the cache.

| Unique misses
| The number of times there was no existing node in the cache.

| Unique hit rate
| The relative number of unique hits with respect to the total number of hits and misses.

| Operator access
| The number of times a BDD operation is called. 

| Operator hits
| The number of times a BDD operation on a node or a pair of nodes was already in the cache.

| Operator misses
| The number of times a BDD operation on a node or a pair of nodes was not in the cache.

| Operator hit rate
| The relative number of operator hits with respect to the total number of hits and misses.

| Swap count
| The number of times two consecutive BDD variables that depend on each other are swapped while reordering the BDD variables.
|===
