...
+
Timing statistics are printed to the console, after execution, just before termination of the tool.
...
This metric is discussed in more detail in the following paper: Sander Thuijsman, Dennis Hendriks, Rolf Theunissen, Michel Reniers, Ramon Schiffelers, Computational Effort of BDD-Based Supervisor Synthesis of Extended Finite Automata, IEEE Conference on Automation Science and Engineering, Pages 486-493, 2019.
+
This statistic is printed to the console, after execution, just before termination of the tool.
...
+
Prints statistics related to the BDD cache.
+
The BDD library that is used implements a cache to speed up the calculations.
For more information, see the <<tools-datasynth-op-cache>> section below.
+
This statistic prints BDD cache related metrics to the console, after execution, just before termination of the tool.
...
| Average chain
| The fraction of 'unique chain' with respect to 'unique access'.
...
| Unique hit rate
| The fraction of 'unique hits' with respect to the total number of 'unique hits' and 'unique misses'.
...
| Operator hit rate
| The fraction of 'operator hits' with respect to the total number of 'operator hits' and 'operator misses'.

| Swap count
| The number of times two consecutive BDD variables are swapped while reordering the BDD variables.
This metric is currently always disabled and thus `0` is printed.
For more information, see the <<tools-datasynth-var-order>> section.
|===
