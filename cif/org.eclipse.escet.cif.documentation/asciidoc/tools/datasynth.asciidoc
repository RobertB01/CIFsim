...
* _Continuous nodes statistics file_: The absolute or relative local file system path to the continuous nodes statistics output file.
...
* _Continuous BDD node usage_ [`bdd-continuous-nodes`]
+
The larger a BDD is, the more memory is needed to store it. 
The size of a BDD can be expressed as the number of nodes used, which is implementation and machine independent.
...
Is printed to the _continuous nodes statistics file_, after execution, just before termination of the tool (see <<tools-datasynth-options,options>> section above).
...
| Unique hits
| The number of times the node was already in cache.

| Unique misses
| The number of times there was no existing node in cache.
...
| Operator hits
| The number of times a BDD operation on a node or a pair of nodes was already in cache.

| Operator misses
| The number of times a BDD operation on a node or a pair of nodes was not in cache.
