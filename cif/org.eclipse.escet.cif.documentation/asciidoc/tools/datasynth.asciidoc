...
The initial order in which the edges are applied is determined by the _Event order_ option (see <<tools-datasynth-options,options>> section above).
Several default orders exist, and it is also possible to define a custom order.
...
+
The initial order of the edges is as they occur in the model.
Input variable are always put after the edges, sorted based on the variable order.

* _reverse model ordering_ (option value `reverse-model`)
+
The initial order of the edges is as they occur in the model, but reversed.
Input variable are always put before the edges, sorted based on the reversed variable order.

* _sorted ordering_ (option value `sorted`)
+
The initial order of the edges is based on the names of the events and input variables.
They are sorted alphabetically in ascending order, based on their absolute names.
...
+
The edges are initially ordered randomly.
If no seed is specified, a random seed is used, resulting in a random random order.
If a seed is specified, a fixed random order is used.
That is, using the same seed again, results in the same random order.
The `SEED` must be an integer number in the range [0 .. 2^64^ - 1].
