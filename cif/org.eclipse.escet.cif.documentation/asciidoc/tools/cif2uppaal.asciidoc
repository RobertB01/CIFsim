...
link:http://www.uppaal.org/[UPPAAL] is a tool modeling, validation, and verification of networks of timed automata with variables.
...
* In Eclipse, right click a `.cif` file in the 'Project Explorer' tab or 'Package Explorer' tab and choose menu:CIF verification tools[Convert CIF to UPPAAL...].

* In Eclipse, right click an open text editor for a `.cif` file and choose menu:CIF verification tools[Convert CIF to UPPAAL...].
...
* Only state invariants are supported.
...
The guards of the self loops express the conditions under which the events are globally enabled (guard wise) in the CIF specification.
