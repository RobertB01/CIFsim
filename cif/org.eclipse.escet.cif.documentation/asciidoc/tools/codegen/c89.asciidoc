...
The <<tools-codegen-chapter-index>> can generate C89 code from a CIF specification.
It is assumed the reader of this page is familiar with the general information of the CIF code generator tool.
This page describes specific information applicable only to C89 code generation.

In general, use of the <<tools-codegen-chapter-c99,C99>> code generator is preferred, as the language contains a decade longer development.
...
The C89 code generator only uses the <<tools-codegen-options,common options>> that apply to all target languages/platforms of the <<tools-codegen-chapter-index>>, it has no options of its own.
...
* The `+...+` in the C89 `+...Type+` name for arrays (fixed length lists) and tuples is the <<tools-codegen-c89-systematic-type-names,systematic type name>> of the array or tuple type.

* Due to <<tools-codegen-preprocessing,preprocessing>> all enumerations are merged into one.
The generated C89 code always has at most one translated `enum`.
