...
The resulting mex file can be used in Simulink (Open the menu:Simulink Library[], select menu:User-defined functions[], and then the menu:S-Function[] block, drag it into the Simulink canvas, open the block , and type the `<prefix>` name into the menu:Name[] field.)
