...
* In Eclipse, right click a `.cif` file in the menu:Project Explorer[] or menu:Package Explorer[] and choose menu:CIF implementation tools[Generate code for CIF specification...].
...
* State/event exclusion invariants (in components as well as locations) are not supported.
...
The continuous variables from the CIF specification in the generated code are updated each execution using the link:http://en.wikipedia.org/wiki/Euler_method[Euler method] for integration.
...
The Euler method can be link:http://en.wikipedia.org/wiki/Numerical_stability[numerically unstable], it can suffer from rounding errors due to the use finite binary representations of real values, and has some other issues as well.
...
The <<tools-cif2cif-chapter-linearize-merge,linearization>> that is used as preprocessing, also <<tools-cif2cif-chapter-merge-enums,merges all enumerations>> into a single enumeration.
