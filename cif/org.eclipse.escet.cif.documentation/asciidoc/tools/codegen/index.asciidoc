...
The CIF code generator can be used to generate implementation code for
various languages/platforms. The generated code is complete, and can be used as
is, without any CIF related library or runtime.

The remainder of this page explains general information about the tool, that
applies to all target languages/platforms. Specific information is available
for each of the target languages/platforms, on separate pages:

* <<tools-codegen-chapter-c89,C89>> (if there is no `C99` compiler)
* <<tools-codegen-chapter-c99,C99>>
* <<tools-codegen-chapter-java,Java>>
* <<tools-codegen-chapter-simulink,Simulink>>, with an example of
<<tools-codegen-chapter-simulink-example,controlling a tank>>
...
* In Eclipse, right click a `.cif` file in the menu:Project Explorer[]
or menu:Package Explorer[] and choose
menu:CIF implementation tools[Generate code for CIF specification...].

* In Eclipse, right click an open text editor for a `.cif` file and choose
menu:CIF implementation tools[Generate code for CIF specification...].

* Use the `cifcodegen` tool in a ToolDef script. See the
...
options that apply to all target languages/platforms:

* _Input file_: The absolute or relative local file system path to the
input CIF specification.

* _Output directory_: The absolute or relative local file system path to the
...
created. By default, the current directory is used.

* _Target language_: Specifies the target language/platform for which to
generate code. By default, C99 code is generated. The following
languages/platforms are supported:
+
...
|===
+
The first column indicates the target language/platform, and is linked to a
...
* _Code prefix_: The code prefix, used to prefix file names, identifiers in
the code, etc. If no prefix is given, the prefix is derived from the input
filename, by removing the `.cif` file extension, if present. This is also
the default. The prefix must be a valid CIF identifier, i.e. consist of only
letters (`a` to `z`, `A` to `Z`), numbers (`0` to `9`), and
underscores (`+_+`), and not start with a number.
...
The CIF code generator supports a subset of CIF specifications. The following
restrictions apply:

* Specifications without automata are not supported.

* Initialization predicates in components are not supported, except if it
...
`string`, enumerations, tuples, and arrays, are not supported. This applies
...
than boolean, integer, real, string, or enumeration values is not supported.

* The use of the addition binary operators on anything other than numeric or
string values is not supported.

* The use of the subtraction binary operators on anything other than numeric
values is not supported.

* The use of the element test and subset binary operators is not supported.

* Projection on anything other than tuples, arrays, and strings is not
...
left) as well as negative indices (counting from the right) are supported.
...
* The `del`, `pop`, `acosh`, `asinh`, `atanh`, `cosh`, `sinh`,
and `tanh` standard library functions are not supported.

* The `empty` standard library function on anything other than arrays is not
supported.

* The `size` standard library function on anything other than strings and
...
This section explains the basics behind the generated code, as they apply to
all target languages. The specific pages that explain the code generated for
specific target languages provide further details.


==== Code overview

The code that executes the edges of the CIF model is called the _event loop_
...
During the execution of the code, variable `time` from the CIF model is
...
How often the code is executed, and whether this is done with a fixed cycle
...
language, and can be found on their respective pages.


==== Event loop

The CIF model is first <<tools-cif2cif-chapter-linearize-merge,linearized>> to
...
called the _event loop_. Once no more events are enabled, the event loop
terminates.


==== Event order

The order in which the code is generated for the different edges (in the
_event loop_), is done in such a way that it matches the order in which
...
iteration of the loop none of the events is enabled, the event loop terminates.


==== Termination

If in the CIF model always at least one event is enabled, the generated code
...
If at least one event is enabled, the event loop is executed again. If during
...
differ per target language, and can be found on their respective pages.


==== Optional execution feedback

Several implementations provide additional information during simulation, such as
...
The generated code cannot guarantee its correct behavior if the inputs from the
...
functioning of the generated code.
...
The continuous variables from the CIF specification in the generated code
are updated each execution
using the link:http://en.wikipedia.org/wiki/Euler_method[Euler method] for
...
used, calculated using the state of the previous execution.

The Euler method can be
link:http://en.wikipedia.org/wiki/Numerical_stability[numerically unstable], it
...
The <<tools-cif2cif-chapter-linearize-merge,linearization>> that is used as
preprocessing, also <<tools-cif2cif-chapter-merge-enums,merges all enumerations>>
