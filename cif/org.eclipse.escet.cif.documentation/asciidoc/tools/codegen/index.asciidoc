...
The order in which the code is generated for the different edges (in the _event loops_), is done in such a way that it matches the order in which transitions are chosen by the <<tools-cifsim,simulator>>.
