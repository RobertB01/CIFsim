...
WARNING: The JavaScript code generation is currently being developed.
...
* There are currently no additional options for JavaScript code generation.
...
JavaScript code generation leads to a set of files being generated in the output folder.
Eventually, we'll have two export options for the JavaScript target, "Complete" and "Simulator".

* Complete: Default, currently the only option available.
This option exports separate HTML, CSS, JavaScript and SVG files.
* Simulator: Currently under construction, this option will export a single HTML file combining all resources, for purposes of demonstrations, sharing and feedback.
...
+
The `frequency` field (in the generated class) can be used to configure how often, in number of times per second, the code should be executed.
