...
The C89 code generator only uses the <<tools-codegen-options,common options>> that apply to all target languages/platforms of the <<tools-codegen-chapter-index>>.
...
This is enabled (`true`) by default.
+
By default, this methods prints information about event transitions that are executed, and states that are reached.
The `doTransitionPrint` and `doStatePrint` fields of the class can be used to configure whether transition and state output, respectively, are printed.
They are enabled (`true`) by default.
If `doStatePrint` is enabled, information is also printed for the initial state.
