...
Thus, if multiple transitions for controllable events are enabled at the same, it does not matter which transitions for controllable event are chosen and in which order, as the end result is the same.
