...
This property ensures that the code is still non-blocking, and this guarantee of synthesis is thus preserved in generated code.
...
For example, the `Supervisor1` automaton in the example is not non-blocking under control, and thus blocking under control.
All locations are reachable.
If `c_act1` is done first, before `c_act2`, the automaton goes through marked location `Done1`.
If instead `c_act2` is done first, before `c_act1`, the automaton does not go through a marked location.
If the every time the choice is made to first do `c_act2` and only then `c_act1`, the automaton never goes through a marked location.
...
As another example, consider the `Supervisor2` automaton below, which is also not non-blocking under control, and thus blocking under control.
Here too, all locations are reachable.
If first `c_act1` is done, then `c_act2`, and finally `u_button_pusehd`, then it cycles without going through a marked location.
