...
* In Eclipse, right click a `.cif` file in the _Project Explorer_ tab or _Package Explorer_ tab and choose menu:CIF implementation tools[Generate PLC code for CIF specification...].
...
Use the automatic option to automatically select the maximum number of bits.
The automatic option is supported for the TwinCAT and S7 <<tools-cif2plc-output-type,output types>>.
...
For S7 <<tools-cif2plc-output-type,output type>>, enumerations and arrays are not supported.
...
For S7 <<tools-cif2plc-output-type,output type>>, arrays are not supported.
...
The S7 output (1200, 1500, 300 and 400) generates code for SIMATIC controllers.
...
By default, if no output directory is specified, it defaults to the input file path, where the `.cif` file extension is removed (if present), and a `+_s7_<ver>+` (`<ver`> being either 1500, 1200, 300 or 400) directory extension is added.
