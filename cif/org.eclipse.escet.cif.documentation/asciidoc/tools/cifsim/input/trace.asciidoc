...
If the trace input mode is used, the menu:Trace input file[] option (menu:Input[] category) can be used to specify the path to the trace file.
...
+
If multiple transitions are possible for the event, due to <<tut-basics-chapter-non-determinism,non-determinism>>, the strict option (see above) determines if and how the simulator proceeds.
