...
During simulation, as long as none of the interactive SVG elements (the SVG elements for which an input mapping exists) are clicked, the corresponding events are never chosen.
...
To prevent such deadlocks, you can use a <<lang-tut-data-chapter-monitoring,monitor>> automaton.
If multiple transitions are possible for that event, the choice (between the transitions for that event only) is deferred to the <<tools-cifsim-input-chapter-automatic,automatic input component>>, which makes a choice based on its own settings.
...
Clicking on either of them results in the corresponding input mapping being applied, and the resulting event being chosen by the SVG input component.
