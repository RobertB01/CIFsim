...
<<tools-cifsim-input-automatic-limitations,limitations>> of using these
options.
...
menu:Automatic mode choice algorithm[]
option (menu:Input[]
category). The following choices are available:
...
transitions if <<tools-cifsim-chapter-env-events,environment events>> are
...
that is used, is <<tools-cifsim-output-chapter-normal,printed>> to the console
...
lead to the same <<tools-cifsim-chapter-traces,trace>>. Seeds are integer
...
can however be influenced using the
menu:Automatic mode time transition duration[]
option (menu:Input[]
...
The option to choose the maximum allowed duration does exactly what it states,
i.e. it chooses the maximum allowed duration for each time transition, until
the next potential state event.
...
automaton to the specification, that could look as follows:
...
that time, i.e. if the new sample is smaller than 3.7.
...
is <<tools-cifsim-output-chapter-normal,printed>> to the console at the start of
...
same <<tools-cifsim-chapter-traces,trace>>. Seeds are integer numbers from the
range [0 .. 2^30^].

The random durations are particularly useful in combination with
<<tools-cifsim-chapter-env-events,environment events>>, as it allows the
...
<<tools-cifsim-chapter-traces,trace>> that the simulator will explore. This
...
_may_ be chosen when the time transitions are interrupted, or at state events,
but that is not guaranteed, as the simulator may choose the time transition
again, after the interruption.

The options provide a lightweight, quick, and easy to use approach to adding
...
may or will occur. One way to accomplish this, is by adding a use case.


=== Undo/reset

The automatic input mode does not support
