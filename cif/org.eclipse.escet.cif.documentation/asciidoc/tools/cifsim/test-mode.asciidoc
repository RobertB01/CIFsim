...
In order to be able to perform automated testing, the menu:Test mode[] option (menu:Output[] category) can be used.
...
* The user is no longer asked to <<tools-cifsim-termination-confirm,confirm termination>> of the simulator, if the the menu:Ask to confirm simulator termination[] option (menu:Simulator[] category) is configured to let the simulator decide whether to ask the user for confirmation or terminate automatically.
