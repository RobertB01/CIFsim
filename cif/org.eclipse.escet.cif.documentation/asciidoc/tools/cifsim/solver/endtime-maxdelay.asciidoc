...
Using the menu:Simulation end time[] option (menu:Simulator[] category), the <<tools-cifsim-termination-endtime,end time>> of the simulation can be configured.
...
Using the menu:Maximum delay[] option (menu:Simulator[] category), the maximum delay amount for a single time transition can be set.
