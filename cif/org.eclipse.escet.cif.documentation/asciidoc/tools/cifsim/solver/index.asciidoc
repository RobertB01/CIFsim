...
These changes are calculated using an link:http://en.wikipedia.org/wiki/Ordinary_differential_equation[ODE] link:http://en.wikipedia.org/wiki/Numerical_ordinary_differential_equations[integrator].
As soon as a guard for an event becomes enabled, time should no longer progress.
To calculate whether (and when) guards change value, a link:http://en.wikipedia.org/wiki/Root-finding_algorithm[root finding algorithm] is used.
Software to solve ODEs, through integration and root finding, are often called ODE solvers.
The CIF simulator uses the link:http://commons.apache.org/proper/commons-math/[The Apache Commons Mathematics Library].
