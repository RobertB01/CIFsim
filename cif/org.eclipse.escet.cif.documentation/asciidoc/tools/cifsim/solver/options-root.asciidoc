...
The menu:ODE solver root finder maximum check interval[] option (menu:ODE solver: ODE root finder[] category) is explained on a separate page.
...
The menu:ODE solver root finding algorithm[] option (menu:ODE solver: ODE root finder[] category) can be used to configure the root finding algorithm to use.
The following algorithms are available:

* Regula Falsi (link:http://en.wikipedia.org/wiki/False_position_method[False position]) method
...
The menu:ODE solver root finder absolute tolerance[] option (menu:ODE solver: ODE root finder[] category) and the menu:ODE solver root finder relative tolerance[] option (menu:ODE solver: ODE root finder[] category), can be used to configure how precise the results of the root finder should be.
...
The menu:ODE solver root finder maximum iterations[] option (menu:ODE solver: ODE root finder[] category) can be used to set the maximum number of iterations to use for root finding.
