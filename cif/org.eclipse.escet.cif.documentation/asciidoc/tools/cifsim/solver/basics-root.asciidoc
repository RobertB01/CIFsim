...
<<tools-cifsim-solver-chapter-basics-int,integration>>, a
link:http://en.wikipedia.org/wiki/Root-finding_algorithm[root finding algorithm]
can be used.
...
Here, the value of continuous variable `x` increases with `0.5` every time
unit. Once the value of `1.5` is reached, the variable is reset to `0.0`.
This process is repeated:

image::{tools-imgsdir}/cifsim/solver/root_simple.png[]

If we look at the data calculated by the numerical integrator, to solve the ODE
problem:

image::{tools-imgsdir}/cifsim/solver/root_simple2.png[]

we see that values are calculated for time points `1.1111` and `5.0`. If we
then also look at the value of the guard, where we use value `-1` for
`false` and `+1` for `true`:

image::{tools-imgsdir}/cifsim/solver/root_simple_guard.png[]

we see that at time `1.1111`, the value of the guard is `-1` (`false`).
At time `5.0` it is `pass:c[+1]` (`true`). That is, the guard changed value
...
interval thus _brackets_ the root.

For the example above, the root is calculated as follows:

image::{tools-imgsdir}/cifsim/solver/root_root.png[]

We start with the interval `[1.1111 .. 5.0]`. The middle point is `3.0555` at which
the guard holds, just like the upper bound (at `5.0`). Thus `5.0` is replaced by
`3.0555`. The middle point of `1.1111` and `3.0555` is `1.5972`. Since the guard holds
for time `1.5972`, upper bound `3.0555` is replaced by `1.5972`. The middle point
of `1.1111` and `1.5972` is `1.3542`. The guard does not hold at time `1.3542`, so the
lower bound of `1.1111` (at which the guard does not hold) is replaced by `1.3542`.
...
While the bisection method is relatively simple, the root finding algorithms
...
the bounds, to get to a satisfactory answer.
