...
To detect such changes during <<tools-cifsim-solver-chapter-basics-int,integration>>, a link:http://en.wikipedia.org/wiki/Root-finding_algorithm[root finding algorithm] can be used.
...
That is, it calculates the exact time point at which the guard function crosses the time axis, and thus has a link:http://en.wikipedia.org/wiki/Zero_of_a_function[root].

indexterm:[ODE,bisection]
indexterm:[ODE,bracketing]
indexterm:[ODE,interval]
One of the simplest root finding algorithms, is the link:http://en.wikipedia.org/wiki/Bisection_method[bisection method].
This algorithm starts with the two time points where the guard has opposite link:http://en.wikipedia.org/wiki/Sign_%28mathematics%29[signs].
