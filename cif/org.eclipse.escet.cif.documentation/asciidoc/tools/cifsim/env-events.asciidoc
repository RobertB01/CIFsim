...
As such, the simulator has an menu:Environment events[] option (menu:Input[] category), that makes it possible to let the simulator know about the _environment events_ (non-urgent events).
...
As value for the menu:Environment events[] option, comma separated _filters_ should be supplied.
