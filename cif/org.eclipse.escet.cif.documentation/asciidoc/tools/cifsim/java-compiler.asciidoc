...
The simulator internally generates link:http://oracle.com/java[Java] code from the CIF model.
...
This compiler has the advantage that is is a faster in actually compiling the Java code, while it has as disadvantage that it has a bit more overhead of accessing the file system.
