...
CIF supports <<tut-data-chapter-discrete-variables,discrete variables>> with <<lang-tut-data-discvar-init-multiple,multiple potential initial values>>, as well as discrete variables with an <<lang-tut-data-discvar-init-any,arbitrary initial value>>.
...
Providing an initialization for a discrete variable with a <<tut-functions-chapter-functions-as-values,function type>> or a <<tut-stochastics-chapter-intro,distribution>> type, is not supported.
