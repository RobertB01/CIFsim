...
+
Prints all target states to the console. This
...
final/deadlock state.

* _The final state_ [`state-final`]
+
Prints the final state to the console, regardless of whether or not the final
state is a deadlock state.
...
+
Prints all intermediate states to the console. Intermediate states are the
states that are used to generate
<<tools-cifsim-output-realtime-frames,intermediate frames>>, for smooth
visualizations. The intermediate states are generated for states between the
start and end states of time transitions, when
<<tools-cifsim-output-chapter-realtime,real-time>> simulation is enabled. Note
...
+
Prints the algebraic variables (derived state) as part of the state. This
...
whether <<tools-cifsim-output-normal-state-filter,state filtering>>
includes or excludes them.

* _The derivatives as part of the state_ [`state-derivs`] (default)
+
Prints the derivatives of the continuous variables (derived state) as part of
...
The derivative of variable `time` is not included. Disabling printing of
derivatives excludes derivatives, regardless of whether
<<tools-cifsim-output-normal-state-filter,state filtering>> includes
or excludes them.

* _The possible transitions (for interactive console choice only)_ [`trans-minimal`]
+
Prints the possible transitions to the console, if the
<<tools-cifsim-input-chapter-console,interactive console input mode>> is used. Multiple possible
...
+
Similar to 'interactive console choice only', but additionally always prints
the possible transitions, if there are multiple possible transitions,
regardless of the <<tools-cifsim-input-chapter-index,input mode>>.

* _The possible transitions (always)_ [`trans-always`]
+
Always print the possible transitions to the console, regardless of the input
...
+
Prints the chosen transitions (the transitions that are about to be taken),
to the console.

* _The interrupted transitions_ [`interrupted-trans`] (default)
+
Prints information about interrupted transitions (time transitions that are
stopped prematurely) to the console.

* _The simulation result_ [`sim-rslt`] (default)
+
Prints the simulation result the console, at the end of the simulation, if
simulation did not result in a runtime error.
...
Prints the seeds used for the random generators to the console, to allow
reproducing the simulation <<tools-cifsim-chapter-traces,trace>>. Only if a
random seed is used, is the seed printed. That is, for user-specified seeds,
the seed is not printed. The following seeds may be printed:
+
...
+
** The seed used for the first random generator for a <<../distr-seeds.asciidoc#,stochastic distribution>>.
...
In the option dialog, each of the different types of output can be enabled and
disabled individually (menu:Normal output[]
option,
menu:Output[]
category).

From the command line, using the `-o` or `--output` option, the names of
...
`-o` or `--output` option is not used. As an example, consider
`-o print,seeds` or `--output=print,seeds`. Only output generated by print
declarations (due to `print`) and output about the used seeds for random
generators (due to `seeds`) is enabled, while all other output is disabled.

The output types that are specified using the option replace the default output
types. However, it is also possible to specify additions and removals relative
to the default output, by prefixing output types with a `pass:c[+]` or `-`
respectively. For instance, when using `-o -print,+trans-always` or
`--output=-print,+trans-always` the default output is used, with print
declarations output excluded from it (due to `-print`), and with always
printing possible transitions enabled (due to `+trans-always`). Replacements
(no prefix) may not be combined with additions/removals (`pass:c[+]` or `-`
prefix).

Specifying a replacement output type twice leads to a warning being printed to
the console. Adding an output type that is already present or removing an
output type that is not present, also leads to a warning being printed.

indexterm:[normal console output,output details]

=== Output details

When printing states, variable `time` is always printed first. It is followed
...
algebraic variables. Except for variable `time`, the state objects are sorted
alphabetically based on their absolute names. For automata with a single
nameless location, the current location is always `+*+`.

For variables with a function type and implicit default values, the
default values may include `defaultValueFunc`, `defaultValueFunc2`, etc.

When printing the possible transitions, the event transitions are printed
first, followed by the `tau` transitions, and the time transition. The event
transitions are sorted alphabetically on the names of the events.
...
By default, when printing states, all state objects are included. Using
the
menu:Normal output state filters[]
option
(menu:Output[]
category), the state objects can be filtered. The
menu:Normal output state filters[]
option only has effect
if states are printed at all.
...
objects of the CIF model, a warning is printed to the console.
...
warning is also printed if the entire state is filtered out.
...
Regardless of whether the filtered result includes them or not, all algebraic
variables and derivatives of continuous variables may be excluded, using the
<<tools-cifsim-output-normal-types,Normal output option>>.

indexterm:[profiling,normal console output]

=== Profiling

If <<tools-cifsim-chapter-profiling,profiling>> is enabled, all normal output is
automatically disabled. That is, profiling suppresses all normal output.
