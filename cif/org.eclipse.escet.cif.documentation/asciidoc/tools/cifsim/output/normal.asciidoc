...
In the option dialog, each of the different types of output can be enabled and disabled individually (menu:Normal output[] option, menu:Output[] category).
...
Using the menu:Normal output state filters[] option (menu:Output[] category), the state objects can be filtered.
The menu:Normal output state filters[] option only has effect if states are printed at all.
