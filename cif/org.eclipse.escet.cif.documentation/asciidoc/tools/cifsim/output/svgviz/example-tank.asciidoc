...
In the menu:XML Editor[] window, you see the position as SVG represents it.
...
The `y`-axis was inverted by using the menu:Flip Vertical[] command from Inkscape's menu:Object[] menu, while the `water` rectangle was selected.
...
In SVG (and thus the menu:XML Editor[] window), we see a new `transform` attribute, `scale(1,-1)` as value, which represents the inversion of the `y`-axis.
