...
Looking at the menu:XML Editor[] window, we can see the bar has a background (box), and a foreground (front) rectangle.
...
SVG however, provides several ways to specify link:http://www.w3.org/TR/SVG11/types.html#DataTypeColor[colors]:

* link:http://www.w3.org/TR/SVG11/types.html#ColorKeywords[Named colors], such as `red`, `yellow`, `lightblue` or `burlywood`.
