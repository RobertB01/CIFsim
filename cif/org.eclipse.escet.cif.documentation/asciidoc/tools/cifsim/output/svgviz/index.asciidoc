...
The simulator however, also supports _SVG visualization_, which allows the user
...
directly corresponds to the way the user sees the system.
...
Below is a list of available examples of SVG visualizations, which show some
often used SVG attributes, as well as some tips and tricks on how to handle
common challenges. The following examples are available:
