...
Consult the official link:http://www.w3.org/TR/SVG11/[SVG specification] for further details.
...
Like SVG, CSS is an international standard developed and maintained by the link:http://www.w3.org/[World Wide Web Consortium] (W3C).
...
In practice, it is usually the easiest to use the menu:Object Properties[] dialog of <<tools-cifsim-output-svgviz-chapter-inkscape,Inkscape>>, to get the properties of the text label.
The menu:Id[] that is then shown, is the id of the `text` element.
...
The expressions however, may also contain references to variables (discrete, continuous, or algebraic), functions, constants, locations, etc.
...
Finally, link:http://www.w3.org/TR/SVG/text.html#WhiteSpace[by default] consecutive spaces (including tabs) are merged together into a single space, and spaces at the beginning and end of the output of a mapping are discarded by SVG.
