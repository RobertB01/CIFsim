...
Both the object properties tab as well as the XML editor can be fixed to the right of the interface or as a floating window.
Simply drag the tab to the preferred location.
...
* Select menu:Layer[Layers and Objects...], to show the _Layers and Objects_ panel.
In that panel, select _Layer 1_ by right clicking on it.
Then click the _Delete layer_ button to remove the selected layer.
