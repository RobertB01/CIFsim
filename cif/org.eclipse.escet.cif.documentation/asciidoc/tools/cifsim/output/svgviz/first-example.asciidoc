...
Start link:http://inkscape.org/[Inkscape], Select menu:Document Properties...[] from the menu:File[] menu, to open the menu:Document Properties[] window.
On the menu:Page[] tab, set the menu:Units[] to menu:px[], the menu:Width[] to `75.00`, and the height to `100.00`.
...
The menu:Object Properties[] window will appear.
Change the menu:Id[] of the circle to `lamp` and click the btn:[Set] button.
Save the image as `lamp.svg`, in the same directory as the `lamp.cif` file.


=== Simulation and the options dialog

Right click the `lamp.cif` file and choose menu:Simulate CIF specification...[] to start the CIF simulator.
...
Using the menu:GUI input[], click the button for btn:[time delay] and then the button for btn:[tau].
...
Close the menu:GUI input[] to terminate the simulator.
You may also close the visualization window.

Now restart the simulator.
In the menu:Simulator[] category, set the <<tools-cifsim-solver-chapter-endtime-maxdelay,Simulation end time>> option to menu:Finite end time[].
You can leave the default value for menu:End time[].
In the menu:Input[] category, set the <<tools-cifsim-input-chapter-index,Input mode>> option to menu:Automatic input mode[].
In the menu:Output[] category, enable the <<tools-cifsim-output-realtime-framerate,Frame rate>> option and keep the default frame rate value.
Click menu:OK[] to start the simulation again.
