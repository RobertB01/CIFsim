...
These shapes are grouped together, using the menu:Group[] command from Inkscape's menu:Object[] menu.
...
To get rid of this, once the person is at the correct initial position, we can ungroup the `person` group ( menu:Ungroup[] command from Inkscape's menu:Object[] menu), and regroup it.
