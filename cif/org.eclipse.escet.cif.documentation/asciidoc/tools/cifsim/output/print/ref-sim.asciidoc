...
For the special targets link:http://en.wikipedia.org/wiki/Standard_streams[standard output stream] (stdout) and link:http://en.wikipedia.org/wiki/Standard_streams[standard error stream] (stderr), the output may get mixed, for instance on the Eclipse console.
...
It can be disabled by reducing the types of <<tools-cifsim-output-chapter-normal,normal console output>> using the menu:Normal output[] option (menu:Output[] category).
