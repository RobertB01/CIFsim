...
WARNING: This page is primarily intended for the developers of the CIF simulator,
and not its end users.

The CIF simulator internally often queries the trajectories computed by the
<<tools-cifsim-solver-chapter-index,ODE solver>>. Due to the limited precision
...
deviations, to avoid accumulation of such deviations.

indexterm:[ulp]
The simulator uses the value of the
menu:Maximum time point tolerance[]
option (menu:Simulation[]
...
link:http://en.wikipedia.org/wiki/Unit_in_the_last_place[ulps]
(_units in the
last place_, or _units of least precision_). One ulp is the difference
...
values of the time points.

The tolerance values chosen using the option must be in the range
[1 .. 2^30^]. The default tolerance is 2^10^ (= 1024) ulps. The default
tolerance should suffice, but if it doesn't, you may
be presented with an error like this:
...
If you get such an error, you can try to increase the tolerance using the
menu:Maximum time point tolerance[]
...
default tolerance doesn't suffice for your model.
