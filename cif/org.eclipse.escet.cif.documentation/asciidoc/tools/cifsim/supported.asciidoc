...
* Time dependent <<tut-data-chapter-stat-invariants,state invariants>> are not supported.
...
Automata with multiple possible initial locations and discrete variables with multiple possible initial values (including `any`) are not supported, unless additional <<tools-cifsim-chapter-init,initialization>> is provided.
