...
If it already exists, all existing `.java` files, `.dat` files, `.edgedata` files and `.locnames` files are removed from the sub-directory first, before the newly generated code is written.

=== Written files

The generated code consists of `.java` files, `.dat` files, `.edgedata` files and `.locnames` files.
...
By default, it is configured to run the `DebugSimulator.java` file contained within the `src` directory.
...
Right click `debug-cif-simulator-generated-code.launch` and select menu:Debug As[Debug configurations...].
Open `debug-cif-simulator-generated-code` under _Junit Plug-in Test_, select the _Arguments_ tab and set _Working directory_ to _Other_.
