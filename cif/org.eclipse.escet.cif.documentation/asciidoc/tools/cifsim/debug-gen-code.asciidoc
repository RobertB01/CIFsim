...
If it already exists, all existing `.java` files are removed from the sub-directory first, before the newly generated code is written.

=== Written files

The generated code consists of Java files only.
This results in several `.java` files being written to the `cifcode` directory.
One of these files is `_DebugSimulator.java`, this file contains the simulation options that were used.

The `org.eclipse.escet.cif.simulator.debug` project contains a launch file (`_cifsim_debug.launch`) that can be used to reproduce the simulation implemented by the generated code.
It assumes the generated code is contained within the `src` directory.
...
* Right click `_cifsim_debug.launch` in the `org.eclipse.escet.cif.simulator.debug` project, and choose menu:Debug as[_cifsim_debug].
...
Simulations not started from the generated launch configuration, generally don't use this option.
...
The default working directory for the simulation performed via the launch configuration is different from the working directory in the original simulation.
As a consequence, additional files, such as configuration files, may not be found.
The working directory can be changed as follows.
In `_DebugSimulator.java` replace the relative path of the `debug-code` option with an absolute path.
Then, right click `_cifsim_debug.launch` and select menu:Debug As[Debug configurations...].
Open `_cifsim_debug` under _Junit Plug-in Test_, select the _Arguments_ tab and set _Working directory_ to _Other_.
Enter the original working directory and click btn:[Apply] and btn:[Debug].
