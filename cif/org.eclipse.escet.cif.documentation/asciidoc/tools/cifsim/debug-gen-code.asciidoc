...
By enabling the menu:Debug the generated code (by writing it to disk)[] <<tools-cifsim-output-chapter-debug,debugging output>>, the generated code can be written to disk, for further debugging.
...
Additionally, an Eclipse launch configuration file (`+_cifsim_debug.launch+`) is written.
...
* Start a sub-Eclipse application, by right clicking on any plug-in project in the workspace, and choosing menu:Run as[Eclipse Application] or menu:Debug as[Eclipse Application].
...
* Simulate the CIF specification, and make sure you enable the menu:Debug the generated code[] option.
...
* Right click the launch configuration in the `src/cifcode` directory, and choose menu:Debug as[_cifsim_debug].
...
The launch configuration that is written to disk with the generated code, uses the menu:Load debug code[] option (menu:Simulator[] category).
