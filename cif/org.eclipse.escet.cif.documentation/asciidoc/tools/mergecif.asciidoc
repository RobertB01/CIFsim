...
Two input variables can only be merged if they have different annotations.
When an input variable is merged with anything but another input variable, the annotations of the input variable are discarded.
