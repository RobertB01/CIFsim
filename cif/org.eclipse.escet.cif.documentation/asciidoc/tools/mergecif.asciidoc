...
Two input variables, or an input variable and a location, can only be merged if they have different annotations.
When an input variable is merged with anything but a location or another input variable, the annotations of the input variable are discarded.
