...
<<tools-chapter-cif-explorer>> and <<tools-eventbased-chapter-product>> tools
however, the parallel composition is not unfolded into a
<<tools-cifsim-traces-state-space,state space>>.

It can for instance be useful to merge a synthesized supervisor with a timed or
hybrid plant, to validate the supervisor against that more detailed plant,
using interactive simulation and visualization on the merged specification.

The specifications that are being merged can <<tools-mergecif-shared,share>>
events and variables, making it possible to have
interaction between the merged specifications.
...
** Select a `.cif` file in the 'Project Explorer' tab or
'Package Explorer' tab by left clicking on it.

** Select additional `.cif` files by left clicking on them while pressing
the
kbd:[Ctrl]
or
kbd:[Control]
...
** Right click one of the `.cif` files and choose
menu:Merge CIF specifications...[].

* Use the Eclipse IDE to merge a single file with itself (allowed, but
practically not that useful):

** Right click a `.cif` file in the 'Project Explorer' tab or
'Package Explorer' tab and choose
menu:CIF miscellaneous tools[Merge CIF specifications...].

** Right click an open text editor for a `.cif` file and choose
menu:CIF miscellaneous tools[Merge CIF specifications...].

* Use the `cifmerge` tool in a
...
* _Input files_: The absolute or relative local file system path to the
input CIF specifications. These are the files to merge.
...
to `merged.cif`. If the merger is executed via the Eclipse GUI, the output
file, if specified as a relative path (or just a file name), is resolved
relative to the directory that contains the first input file.
...
If only one input file is provided to the CIF merger, essentially that single
...
The supervisor specification contains three _skeletons_: groups that only
...
The timed specification contains the same plants, with the same events, but
...
and the supervisor thus controls the timed plants.
...
The example above shows how shared events can be used to link (or couple)
multiple specifications through merging.

Similarly to the way events can be merged, it is possible to merge input
...
location. The input variable is then a sort of _skeleton_ definition that is
merged with an actual definition (for instance an algebraic variable), which
defines the value.

There are thus two ways for merged specifications to become linked (or
coupled): by means of shared events and by means of shared variables (including
constants).

For further details and restrictions, see the
<<tools-mergecif-compatibility>> section.
...
In general, objects (components, declarations, etc) that only occur in one
...
merged with all other objects, i.e. not all objects are _merge compatible_.
...
For components (i.e. groups and automata) that are present in more than one
...
As the button/lamp example above shows, automata can be merged with groups
...
functions can not be merged with automata.

Groups can also be merged with other groups. Automata however can not be merged
with other automata, as that may lead to the merge of conflicting behavioral
specifications.

Events can be merged with other events. Each event must then either be a
...
controllability in different specifications, can also not be merged.

Constants can be merged with other constants. However, the constants must then
...
for equality (e.g. function types), are not supported (can not be merged).

As mentioned in the <<tools-mergecif-shared>> section, an input variable can
...
with other objects that have the exact same type.

A location can be merged with an input variable that has a boolean type. The
input variable is then a placeholder for the location, indicating whether the
location is the current location of its automaton.

It is not allowed to merge two algebraic variables, two discrete variables, or
two continuous variables, as that may lead to the merge of conflicting values.

In general, only one of the specifications defines the variable as a concrete
...
declare it as an input variable (essentially a _placeholder_). It is however
allowed to merge two or more constants with the same type and the same value.

Type declarations can be merged with other type declarations, as long as they
have the exact same type.

Enumerations can be merged with other enumerations, as long as they are
compatible. Two enumerations are compatible if they have the same number of
literals, with the exact same names, in the same order.

Enumeration literals can only be merged as the result of the enumerations of
which they are a part being merged. Merging two literals from different
enumerations (which then have different names) is not supported.

Type declarations and enumerations can be merged, as long as the type of the
type declaration is compatible with the enumeration.

User-defined functions can never be merged with other objects of the same name.

indexterm:[CIF merger,invariants]

=== Invariants

Merging a group with an automaton, may change the supervisory kinds of the
...
In the original group `x`, the invariant did not have a supervisory kind.
After merging, the invariant still does not specify a supervisory kind.
However, since it is now specified in an automaton `x` of kind `plant`, the
invariant <<lang-tut-extensions-synthesis-invkind-implicit,implicitly>>
becomes a plant invariant as well.

When an invariant gets an implicit supervisory kind as a result of merging,
the merge tool prints a warning to the console.

To get rid of the warning, the invariant in group `x` can be declared
explicitly as being a `plant` invariant. The supervisory kind then matches
the supervisory kind of automaton `x`. When the group and automaton are
merged, the invariant keeps its explicit supervisory `plant` kind. The merge
...
However, in this last case, no warning is printed to the console.

indexterm:[CIF merger,merge problems]

=== Merge problems

Even when two or more CIF specifications are merge compatible, as described
...
But this merged specification is invalid, as `x` is defined in terms of
`y`, which is defined in terms of `x`, leading to a loop (definition/use
cycle).

Another example of merge problems is two CIF specifications that have SVG
...
mappings, but in the merged specification there are.
