...
See the link:https://eclipse.org/escet/tooldef[ToolDef website] for more information on ToolDef.
...
A script can also be used to execute a single tool, specifying the parameters used when executing it.
...
Another use of scripts is to repeatedly execute the same tool, but with different parameters.
A good example is <<tools-cifsim-chapter-repeated-simulation,repeatedly simulating>> a model.
Instead of manually simulating a model with <<tut-stochastics-chapter-intro,stochastics>> hundreds or even thousands of times, which can be very time consuming, a script can be used to automate this task.
...
It is part of the menu:Simulator[] category:

image::{tools-imgsdir}/scripting/option_dialog_cmdline_link.png[]

There, it is listed as menu:Simulator end time[] option.
