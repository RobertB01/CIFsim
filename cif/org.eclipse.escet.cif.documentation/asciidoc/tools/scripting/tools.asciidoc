...
This page provides an overview of the CIF tools that are available in the
`cif` library that is registered with ToolDef by CIF.

=== Tools

The following table shows the relation between the names of the ToolDef tools
in the library and the tools of the CIF tool set:
...
For every CIF tool, two ToolDef tool variants are available in the `cif`
ToolDef library. All the CIF tools in the `cif` ToolDef library have
...
`toolname` instead of the actual tool name (e.g. `cifsim`):
...
All the tools return an integer value. This value is the exit code of the
application. A zero (`0`) exit code means the application terminated without
