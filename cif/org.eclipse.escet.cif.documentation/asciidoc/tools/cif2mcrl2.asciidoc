...
link:https://www.mcrl2.org/[mCRL2] is a tool for verifying properties of the model.
...
* Discrete variables must have type `bool`, `enum`, or `int`.
...
* Boolean literals, boolean-typed discrete variables, and operators `and`, `or`, `==`, `>=`, `>`, `+<=+`, `<`, `!=`, and `not` on booleans are allowed.

* Enumeration literals, enumeration-typed discrete variables, and operators `==` and `!=` on enumerations are allowed.

* Integer literals, integer-typed discrete variables, binary operators `pass:c[+]`, `+*+`, `-`, and unary operators `-` and `pass:c[+]` on integers are allowed.
...
That is, if an empty list is specified, no variables are added.
...
* The 'empty' option value selects no variables.
On the command line or in ToolDef scripts, when using the `-r` short option, use `''` for the 'empty' option value.
...
* The option value `++*1*+` selects all variables with a `1` at any position.
