...
mCRL2 specification.
link:http://www.mcrl2.org/[mCRL2]
is a tool for verifying properties of the model.
...
menu:CIF verification tools[Convert CIF to mCRL2...].

* In Eclipse, right click an open text editor for a `.cif` file and choose
menu:CIF verification tools[Convert CIF to mCRL2...].

* Use the `cif2mcrl2` tool in a
...
* _Input file path_: The absolute or relative file system path to the input
CIF specification.

* _Instance tree definition_: mCRL2 uses a tree of processes which you can
...
<<tools-cif2mcrl2-instancetree-syntax,instantiation tree syntax>> Section.

* _Enable debug output_: The transformer derives information about used and
...
same name as the input file, except first the `.cif` extension is removed
(if present), and a `+_dbg.txt+` extension is appended. The name of the
output file can be changed with the _Debug output file path_ option. Setting
the latter option enables this option.

* _Debug output file path_: The absolute or relative file system path to use
...
output file path to the given path. It also enables the _Enable debug
output_ option.

* _Output file path_: The absolute or relative file system path for writing
the generated mCRL2 output file. By default, the output file path is the
same as the input file path, but with the `.cif` extension removed (if it
exists), and the `.mcrl2` extension added. By setting this option, the
default is overridden by the given value.

* _Generate 'value' actions_: To query the value of variable `x`,
a `+value_x+` action can be used. By default, such actions are generated for
...
<<cif2mcrl2-value-select>> Section for more details.

indexterm:[CIF to mCRL2 transformer,supported specifications]

=== Supported specifications

The CIF to mCRL2 transformer supports a subset of CIF specifications. The
...
* Initialization predicates are not supported, except for initialization
predicates in a location.

* Invariants are not supported.

* Marker predicates are ignored.

* Discrete variables must have type `bool` or `int`. The latter may have
a range.
...
* The transformation translates the range of integer variables if used, but it
does not enforce the maximum and minimum integer value of CIF.
...
* All expressions and sub-expressions must be of type `bool` or `int`. The
latter may have a range.

* Boolean constants, discrete variables, and operators `and`, `or`,
`==`, `>=`, `>`, `+<=+`, `<`, `!=`, and `not` are allowed.

* Integer constants, discrete variables, binary operators `pass:c[+]`, `+*+`, `-`,
and unary operators `-` and `pass:c[+]` are allowed.

However, some of these limitations are resolved by preprocessing of the
specification, as explained in the next section.
...
The CIF to mCRL2 transformer generates a `value` action for each variable by
...
tool. To eliminate these unneeded `value` actions, the CIF to mCRL2
transformer has the _Generate 'value' actions_ option.

The option takes a comma separated list of variable patterns. Each pattern
can add or remove variables. If the pattern starts with a `pass:c[+]` character,
variables are added. If the pattern starts with a `-` character, variables
are removed. If the pattern start neither with a `pass:c[+]` nor with a `-`
character, variables are added. The main part of each pattern is the name of
the variable to add or remove. In addition, you can use the `+*+` character
as a shorthand for 'zero or more arbitrary characters', allowing you to write
abbreviations, and match several variables at the same time.
...
Below a number of example option values, and what variables they select from
the above list.

* The option value `B.x,+A.z` selects variables `B.x` and `A.z`.

* The option value `++*1+` matches all variables that end with a `1`, which
is variable `B.y1` only (Variable `A.p12` does have a `1` in it, but
not at the end.)

* The option value `++*1*+` matches all variables with a `1` at any
position. This matches both `A.p12` and `B.y1`.

* The option value `++*,-B.*+` selects all variables, except those in
automaton `B`. (`+*+` adds all variables, then `+-B.*+` removes all
variables that start with `B.`, that is all variables in automaton `B`.)

* The option value `+-*+` removes all `value` actions, since the pattern
...
In mCRL2, a behavior process is required for every automaton in the input, and
...
All these processes must be instantiated using the parallel composition
...
combinations of actions for the mCRL2 tool to handle.

A different solution is to instantiate smaller groups of processes at a time,
...
Which processes and groups are taken together at each level in the tree has a
...
The CIF to mCRL2 transformer does not perform analysis to decide how processes
...
order of grouping. For this reason, the _Instance tree definition_ option
exists. It allows you to define which processes should be taken together at
each level in the instantiation tree.

The current implementation has limits with respect to ordering. Automata
...
variable to use at each level is free).

The option expects absolute names of all automata in the specification and
...
Processes for the mentioned CIF elements are instantiated. Sub-groups of
processes are written by surrounding them with parentheses. For example:
...
takes automata `A`, `B`, `C` together. One level higher, variable
`A.x` is added, and finally variable `B.y` is added at the highest level.
Another order is:
...
First processes `A` and `B` are combined, then process `C` is added, and
finally variables `B.y` and `A.x` at the top-most two levels.

indexterm:[CIF to mCRL2 transformer,debug output]

=== Debug output

The transformer analyzes use of the variables by each automaton, and computes
...
enabling the _Enable debug output_ option. As an example, consider the
following artificial CIF specification:
...
This specification has two automata `P` and `Q`, and two shared variables
`P.x` and `Q.y`. The ranges of the variables are important, mCRL2
...
Variable `zero` is not shared, only automaton `Q` uses it. The transformer
moves it to the `Q` process. Normally however, you would use a constant
instead of a variable.

The instance tree used in the transformation is `( ( P Q ) P.x ) Q.y`. The
debug output for `P` is:
...
Automaton `P` has one `c` edge, which checks the value of `Q.y` but it
does not change its value. It has two `a` edges, only the `a` edge in
location `p1` reads `P.x` (to compute `x + 1`). Both edges assign a new
value to `P.x`.

In a similar way, the node information for automaton `Q` is computed. The
result is:
...
The automaton only checks values rather than writing them. As you can see,
variable `P.x` is not mentioned with event `a`, since there is no edge in
the automaton with event `a` that accesses variable `P.x`.

The node that merges both automata, i.e. `( P Q )`, has the following
formation:
...
Variable use by each event is simply merged. In this example there is no
overlap, but that works as you would expect, `SOMETIMES` overrides
`NEVER`, and `ALWAYS` overrides `SOMETIMES`.
...
At `( P Q ) P.x` node, the merge of the variable `P.x` with the two
automata, this results in:
...
In this node, variable `P.x` is also available. The variable process does
not perform events, so the event variable table of node `1.1.1` is copied.
The only exception is the additional line `P.x` for event `c`. The reason
for this line is that while `( P Q )` may not use `P.x` in combination
with event `c`, other automata higher up in the tree may still need that
variable. (In a future version of the transformer tool, it may be detected
that such use never happens, and there is no need to add the line here.)

Node `1.2` is the variable process of `Q.y`. It looks the same as node
`1.1.2`, except for the name of the variable.

Finally the top-node that merges `( P Q ) P.x` with `Q.y`, bringing
everything together, looks like:
...
The `Q.y` variable is now also available. The event variable table is again
copied. Since all events already used variable `Q.y` no additional lines
were added.
...
A variable process in mCRL2 handles shared access to a variable. For variable
`P.x` in the above example it looks like:
...
The name of the actions all end with the name of the variable. If the CIF
specification has two or more variables with the same name, a number gets
appended to make each variable unique in mCRL2.

A behavior process is generated for each automaton instance. Like the
...
write actions for synchronizing with the variable processes:
...
Locations are encoded in a `struct` and variable read and write actions are
added as needed.

The instantiation tree is dominated by the action operators needed for
synchronizing and restricting actions. The `+value_...+` actions are allowed
up the to top level node, to give access to variable values in the
verification. This leads to:
