...
That is, the left hand sides of the assignments on edges (the addressables) must be single variables without projections.
...
For instance, the following edge updates:
