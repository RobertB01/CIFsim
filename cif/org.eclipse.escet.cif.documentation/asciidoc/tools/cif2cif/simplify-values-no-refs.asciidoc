...
CIF to CIF transformation, but without simplifying reference expressions.
...
The CIF to CIF transformation to <<tools-cif2cif-chapter-simplify-values>>,
transforms the following specification:
...
This transformation however, does not take the values of declarations into
...
specification into the following simplified specification:
...
This variant can be useful, as it prevents the use of constants from being
...
comparison to a single boolean value.

Another issue related to simplification of constants, is that inlining them
...
The reference expressions that are not simplified (filled in) include among
