...
instantiations, by actually instantiating the component definitions.
...
* Phase 1: Find the component definitions without any component definitions
and/or component instantiations in them.

* Phase 2: Instantiate the component definitions found in phase 1, by putting
...
variables for the algebraic parameters.

* Phase 3: Replace uses of component, event, and location parameters by the
actual arguments provided for the instantiation.
...
Absolute references may be needed to express the results of this
...
Note how the invariant in component `a.x` can not refer to constant `x`
directly, as `x` refers to component `a.x` in that context. Therefore, the
scope absolute reference `.x` is used instead.
...
Since component definitions are shortcuts for components, eliminating
them could result in an increase of the size of the specification.
