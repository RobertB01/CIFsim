...
This CIF to CIF transformation replaces implicit automaton `self` references
by explicit automaton references.
...
All implicit automaton `self` references are replaced by explicit automaton
references. For instance, consider the following CIF specification:
...
Since both references are single expression nodes, this transformation does not
increase the size of the specification.
