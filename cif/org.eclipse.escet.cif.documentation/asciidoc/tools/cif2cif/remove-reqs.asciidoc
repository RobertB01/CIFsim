...
<<lang-tut-extensions-synthesis-plant-req-sup,requirements>> from the
specification. By removing all requirements from a CIF specification with both
<<lang-tut-extensions-synthesis-plant-req-sup,plants>> and requirements, only the
plants remain. The resulting plants can then be <<tools-chapter-mergecif,merged>>
with a <<lang-tut-extensions-synthesis-plant-req-sup,supervisor>> or other
controller.
...
* Using (referring to) requirement automata or declarations from requirement
automata from outside requirement automata.
...
referenced no longer exist. For instance, consider:
...
When requirement automaton `req` is removed, the `switch` expression can no
longer refer to that automaton, or to its locations. Removing the requirements
from this CIF specification is not supported.
...
Both <<lang-tut-extensions-synthesis-autkind,requirement automata>> and
<<lang-tut-extensions-synthesis-invkind,requirement invariants>> are
...
Since information is removed from the specification, the specification can only
become smaller as a result of this transformation.
