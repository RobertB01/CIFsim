...
continuous variables, as well as local variables of functions.
...
The default values for all types are simple values. The only exception are
...
function is reused. The functions are named `+_f+`, `+_f2+`, `+_f3+`, etc.
...
If the names of the functions that are introduced, conflict with already
existing declarations, they are renamed. For instance, if `+_f2+` is already in
use, `+_f22+`, `+_f23+`, etc, are used instead. If renaming takes place, a
...
Since values are added, this transformation increases the size of the
...
value can significantly increase the size of the model.

indexterm:[add default initial values,optimality]

=== Optimality

The default values for all types are as primitive as they can be, and can
...
a function already existed in the original specification.
