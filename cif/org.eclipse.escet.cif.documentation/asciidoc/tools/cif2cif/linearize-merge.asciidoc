...
<<tools-cif2cif-linearize-merge-renaming,if possible>>.
...
<<tools-cif2cif-linearize-merge-nondet,non-deterministic choice>> may be
eliminated, ensuring that the model size of the resulting specification is
near-linear <<tools-cif2cif-linearize-merge-size,compared to>> the model
...
<<tools-cifsim-input-chapter-automatic,first transition>>. Linearization eliminates some of the
...
Related to this, are dummy updates, which are added to ensure that the correct
updates are taken. For instance, the following specification:
...
<<tools-cif2cif-linearize-merge-nondet>> section above explains, if
...
linear in the number of edges as well.
...
For original edges with multiple events however, the guards and updates are
...
If multiple senders and multiple receivers (individual send/receive edges, not
...
From the above, it should be clear that while the specification may increase
...
projections by tuple index projections.
