...
For algebraic variables with a single equation in a component or automaton,
...
generation of the 'if' expression is suppressed.

The derivatives of continuous variables are transformed in a similar way. The
derivative ends up in the declaration of the continuous variable.
...
The right hand sides of the equations are simply moved. This does not influence
...
specification. The increase however, is linear in the number of equations.

indexterm:[eliminate equations,optimality]

=== Optimality

This transformation introduces 'if' expressions for algebraic variables and
...
references, apply the <<tools-cif2cif-chapter-elim-locs-in-exprs>> CIF to CIF
transformation.
