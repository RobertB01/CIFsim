...
* _Header text file path_: The absolute or relative local file system path to the text file to include at the start of the generated PLC program.
Relative paths are resolved relative to the CIF file specified in the _Input file_ option.
The program recognizes text patterns `+${app-name}+`, `+${app-version}+`, `+${time-stamp}+` and `+${brief-explanation}+`.
They get replaced by the name of the PLC code generator application, its current version number, the current time/date and a short explanation how to interpret CIF terminology in the PLC program respectively.
...
A more complete check can be done by checking the finite response property of the input specification with the <<tools-chapter-controller-checker,controller checker>> tool.
