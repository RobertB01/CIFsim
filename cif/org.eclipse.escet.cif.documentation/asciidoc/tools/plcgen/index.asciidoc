...
The CIF PLC code generator for creating IEC 61131-3 compliant **P**rogrammable **L**ogic **C**ontroller (PLC) code.
It takes a CIF specification and creates real-time PLC code that can read values from external inputs, evaluate the state and perform event transitions, and finally write values to external outputs.
...
Currently, the program cannot generate code for arrays for these systems.
Therefore, all forms of CIF lists are forbidden for these target type.
+
See the <<tools-plcgen-chapter-s7,S7 PLC output>> section for details on how to transfer generated code to the PLC system.
* _TwinCAT_.
See the <<tools-plcgen-chapter-twincat,TwinCAT PLC outut>> section for details on how to transfer generated code to the PLC system.
...
In CIF, by changing the value of connected discrete variables, external outputs will receive those new values.
