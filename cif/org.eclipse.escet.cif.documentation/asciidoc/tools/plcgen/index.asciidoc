...
The resulting code is written in the Structured Text programming language, one of the programming languages that is part of the IEC 61131-1 standard.
...
To successfully deploy the generated code at a real system, the CIF model should behave correctly under all circumstances and the used PLC should have a sufficiently short cycle-time to catch all input changes.
...
However, as the PLC code has different needs for names, the names from the CIF specification are not always used as-is.
...
This allows to obtain the original CIF element source for a name in the generated PLC code.
...
Th PLC code generator output contains a single program called `MAIN`.
...
Unless overridden with options, the target also affects the shape of lower level details such as the form of the function calls, selected size of integers, and other configuration that tends to be different fro different PLC systems.
Last but not least, the target affects how the generated output gets written to files.

The PLC code generator knows the following targets:
...
The generated PLC code will automatically read all input at the start of a PLC cycle and store it in the connected input variables.
The values of the variables can be used in a guard or a computation, or get copied to other variables.
At the end of a cycle, outputs are written in a similar way by using the value of the connected discrete variables.
...
* The third column contains the absolute name of the CIF variable connected to the I/O address.
...
Furthermore, the behavior of the generated PLC code must match the behavior of the CIF model.
Hence, all continuous variables in the CIF model must be decreasing timers running to zero.
The following conditions therefore apply to continuous variables to be supported by the PLC code generator:

* The derivative of a continuous variable must always be `-1` or `-1.0`, and this value must be statically decidable.
* Values used in comparing, initializing, or assigning continuous variables have a unit of one second.
...
As a more elaborate example, the following automaton will enable the `timeout` edge 10 seconds after the last `reset` event:
...
However, some parts are not supported by any PLC system.
...
The PLC code generator may change enumerations or plain numbers in such a case.
...
Such events must be performed as well in the same cycle, since in CIF time cannot progress if an edge is enabled.
