...
In such annotations, design related information can be written about the element they are attached to.
...
The PLC code generator understands documentation annotations of specifications, automata, variables, locations, and edges.
Indirectly it also understand documentation annotations of requirements, since it uses CIF conversions to convert these to one of the previously mentioned CIF elements.
All documentation annotations that it finds in the input specification are copied into the generated PLC code as comments, where appropriate.
...
As an example, the following automaton will enable the `timed_out` event 10 seconds after the last `reset` event:
...
Below are a few examples of what is generated in the PLC code.
The code has more places where documentation is shown.
For all details, please convert the example yourself and check what the PLC code generator produces.

.Global overview description of the specification.
[source,c]
----
(*------------------------------------------------------
 * Model overview:
 ...
 *
 * - Controllable event "timed_out".
 *   Enabled when the system has been active long enough.
 *------------------------------------------------------ *)
----

.Code to test whether event "timed_out" can occur.
[source,c]
----
(***********
...
.Code that resets the timer while in the active location.
[source,c]
----
