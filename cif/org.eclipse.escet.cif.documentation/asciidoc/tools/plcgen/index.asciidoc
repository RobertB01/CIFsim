...
NOTE: This application is in the **experimental** stage.
...
The current generator is in a feature-complete state (all parts are there), and is considered a good starting point for further improvements.
However it still lacks in tuning towards specific PLC models.
Also many parts can be improved or extended.
...
The CIF PLC generator can be started in Eclipse in the following ways:

* Right click a `.cif` file in the _Project Explorer_ tab or _Package Explorer_ tab
and choose menu:CIF implementation tools[Generate CIF PLC code for CIF specification (experimental)].

* Right click an open text editor for a `.cif` file and choose menu:CIF implementation tools[Generate CIF PLC code for CIF specification (experimental)].
...
The code generator needs a CIF specification and an IO table file as input.
It also needs a destination where to write the result.

As different PLC systems have different expectations and limitations in what the accept as input, behavior of the PLCgen generator heavily depends on the chosen PLC target.
This is configured with then _PLC target type_ option:
...
Other important settings are the _Input file_ path for the CIF specification, the _I/O table file_ path that contains the connections to the external sensors and actuators, and the _Output file_ for within the generated PLC code:

* _Input file_: The absolute or relative local file system path to the input CIF specification.

* _I/O table path_: The absolute or relative file system path to the input I/O connections.
For more information, consult the <<tools-plcgen-io-table-file, io table>> Section.

* _Output file_: The absolute or relative local file system path to the output file or directory.
The result also depends on the chosen <<tools-plcgen-targettype, target type>>.

=== Tailoring the output of the code generator

Additional options exist to tailor the output of the code generator:

* _Simplify values_: Enable this option to simplify values and expand constants (except for constants used for locations, see _Convert enumerations_ below) in the generated PLC code.
...
By enabling this option, the generator prints a warning whenever this happens, allowing to obtain the original source for a name in case of confusion.
...
* _Convert enumerations_: Locations in the CIF specification are converted to enumeration values.
Using enumeration values improves readability of the PLC code, and enables linking back to the CIF specification.
If the target type does not support enumerations, the enumeration values are converted to constants.
If constants are also not supported, the constants are converted to numbers.
This option forces these conversions.

* _PLC maximum iterations_: The <<tools-chapter-controller-checker,controller checker>> should be used to ensure that the PLC code generated by PLC generator will always finish its PLC cycle by checking that the maximum number of events that cane be done in a single cycle is finite.
...
Default value of this option is `100` iterations.
