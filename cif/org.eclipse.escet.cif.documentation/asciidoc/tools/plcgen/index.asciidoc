...
The bounded response and confluence checks of the <<tools-chapter-controller-checker,controller properties checker>> should be used on the specification to check that this is always the case.
...
** The word `resp`, which means that the PLC code generator takes the loop bound limit from the annotation of the <<tools-chapter-controller-checker,controller properties checker>> as the limit value.
...
** The word `inf`, for 'infinite', which means that the PLC code keeps trying to perform events with a limit, until no more (uncontrollable or controllable) events are enabled.
...
(The additional loop tries all events it should try, and at the end then reaches the conclusion that none of them could be performed.)
...
Default behavior of this option is `resp,resp`, which aims to use the bounded response numbers computed by the <<tools-chapter-controller-checker,controller properties checker>> for both the uncontrollable and the controllable events.
...
* Perform as many uncontrollable events as possible (within loop limits, see the _PLC maximum iterations_ option for details).
