...
An upper limit value is either the word `inf` to get an unlimited number of allowed loops, or a positive integer number maximum number of times that the loop can be done.
