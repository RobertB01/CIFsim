...
Default path is derived from the CIF _Input file_ option value by replacing the `.cif` extension of the path if it exists, and adding the `.csv` extension.
+
...
* No multiple assignments to (parts of) the same variable, for example no `x[0], x[1] = 2, 3;`.
...
* No sets, dictionaries, strings, stochastic distributions, or function types.
* No operations that produce values of the forbidden types: casting from or to a string, dictionary literal values, projection operators on dictionaries, non-array lists or strings, set literals, string literals, and list or array slicing.
* No addition, addition, subtraction, or comparisons between lists or arrays.
* No comparisons between tuples.
* No use of variable `time`.
* No non-array lists.
* Array lists only when the target supports them.
