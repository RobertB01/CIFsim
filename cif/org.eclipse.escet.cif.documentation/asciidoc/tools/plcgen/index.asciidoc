...
* Each assignment of a new vale to a continuous variable should be separate. That is, use `do c:= 1.0, d := 2.0` on edges.
Other forms like `do (c, d) := (1.0, 2.0)` are not allowed.
* Values used to initialize a continuous variable, or to assign to a continuous variable must be larger or equal to zero.
