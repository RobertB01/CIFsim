...
The bounded response and the confluence checks of the <<tools-chapter-controller-checker,controller properties checker>> should be used on the specification to check that this is always the case.
...
** The word `inf`, which means infinite, try to perform events until all (uncontrollable or controllable) events are blocked.
