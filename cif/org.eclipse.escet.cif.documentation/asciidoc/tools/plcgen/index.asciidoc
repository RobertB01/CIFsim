...
** Use _Convert enumerations to integers_ (`--convert-enums=ints`) if the target PLC neither support enumerations nor supports constants.
...
* For each automaton with two or more locations, a new discrete variable is introduced to track its current location.
Its name is derived from the name of the automaton.
Location names are converted to enumeration literals, allowing for useful named values to be assigned to the new variable.
...
In general, the program attempts to preserve as much of the original CIF specification as possible.
For example, names of elements in CIF are also used to the converted equivalents in the generated PLC program.

Some parts are not supported by any PLC system.
For example, multiple automata and synchronizing on events or channels is unknown in a PLC system.
For these cases the generator produces significantly different code, compared with the CIF specification.
For taking an event transition, all edges are grouped by event instead of grouping by automaton.
...
For these cases, the generator tries to generate the form that fits best within the abilities of the PLC system.
Enumerations may be preserved, or get converted to constants or plain numbers, depending on what the targeted PLC system supports.

From a bird's-eye view, the program generates one main program that performs the entire PLC cycle.
