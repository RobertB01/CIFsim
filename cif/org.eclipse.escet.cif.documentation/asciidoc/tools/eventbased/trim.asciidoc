...
The input is a `.cif` file with a single automaton, and the tool produces a
new `.cif` file with the trim automaton. If the automaton in the input was
already trim, all locations are preserved.
...
menu:CIF synthesis tools[Event-based synthesis tools > Apply trim...].

* In Eclipse, right click an open text editor for a `.cif` file and choose
menu:CIF synthesis tools[Event-based synthesis tools > Apply trim...].

* Use the `ciftrim` tool in a
...
`+_trim.cif+` file extension is added. The `trim` part of the default
extension depends on the _Result name_ option.

* _Result name_: The name to use for the trim automaton. If not
specified, defaults to `trim`. Also affects the _Output file_ option.
