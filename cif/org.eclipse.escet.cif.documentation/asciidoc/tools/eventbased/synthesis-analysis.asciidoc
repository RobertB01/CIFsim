...
The bottom button labeled _Search state_ is used to search the information about a supervisor state selected at the left.
The _Jump to initial state_ button resets the automata locations to their initial state, useful when you want to start again.
The middle button is disabled now, but allows you to go back to the previous state.
...
When you press _Search_, the tool analyzes the selected state at the left.
...
If you want to know what states `#9` or `#8` are, you can enable the _Display full state_ at the top-right, and the tool will display the full list of automaton locations with each state, just like the first line in the report.
...
* In Eclipse, right click a `.synth_dump` file in the 'Project Explorer' tab or 'Package Explorer' tab and choose menu:Analyze Synthesis Result[].
