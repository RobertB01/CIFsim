...
The link:http://www.yworks.com/en/products/yfiles/yed/[yEd Graph Editor] is an application that can be used to view, create, edit, arrange, import and export various kinds of diagrams.
...
* In Eclipse, right click a `.cif` file in the 'Project Explorer' tab or 'Package Explorer' tab and choose menu:Convert CIF to yEd diagram...[].
...
Changes with respect to the textual representation include among others elimination of <<tut-reuse2-chapter-imports,imports>>, and the loss of comments and textual layout.

All components (<<tut-basics-chapter-automata,automata>>, <<tut-reuse2-chapter-groups,groups>>, automaton <<tut-reuse2-chapter-aut-def-inst,instantiations>>, and group instantiations) and components <<tut-reuse2-chapter-aut-def-inst,definitions>> (automata and group definitions) are visualized in a hierarchical manner, as rectangles contained in other rectangles.
...
For the declarations (e.g. <<tut-data-chapter-discrete-variables,variables>>), <<tut-data-chapter-stat-invariants,invariants>>, I/O declarations (e.g. <<tools-cifsim-output-svgviz-chapter-cifsvg,SVG declarations>>), etc a block of code is used.
...
The <<tut-data-chapter-init-preds,initialization>> of the location (if present and not `false`) is visualized as an incoming arrow without source location, with a label indicating the initialization predicates if they are not `true`.
...
The events, guards, `now` keywords (for <<tut-time-chapter-urgency,urgent>> edges), and updates are put as labels on the edges.
...
The diagram can be used to see which events are used by which <<tut-basics-chapter-automata,automata>> (are in the <<tut-basics-chapter-alphabet,alphabet>> of those automata).
It can also be used to see which data (variables, locations, etc) exist in the model and where they are used.
Furthermore, it can also be used to see how events and data are passed via automaton and group <<tut-reuse2-chapter-aut-def-inst,instantiations>>.
Events, variables, and locations can thereby potentially be renamed if the <<tut-reuse2-chapter-aut-def-params,parameters>> of the corresponding automaton or <<tut-reuse2-chapter-groups,group>> <<tut-reuse2-chapter-aut-def-inst,definitions>> have different names than the events, variables, and locations that are passed via the instantiations.
...
Event <<lang-tut-basics-automata-events,declarations>>, <<tut-data-chapter-discrete-variables,discrete variables>>, <<tut-time-chapter-continuous-variables,continuous variables>>, <<tut-reuse1-chapter-algebraic-variables,algebraic variables>>, <<tut-reuse2-chapter-input-variables,input variables>>, and <<lang-tut-basics-automata-location,locations>> are all visualized as rectangles with a thick border.
The text in the rectangle is a keyword (`event`, `disc`, `cont`, etc), followed by the name of the declaration.
For <<tut-channels-chapter-intro,channels>> (events with a data type), the name is followed by `!?~`.

Event, location, and algebraic <<tut-reuse2-chapter-aut-def-params,parameters>> are visualized as octagons with a thin dashed border.
The text in the rectangle is a keyword (`event`, `location`, etc), followed by the name of the parameter.
For <<tut-channels-chapter-intro,channels>> (events with a data type), the name is followed by the <<lang-tut-reuse2-aut-def-param-kinds-event-restrictions,event parameter usage restriction flags>> (e.g. `!` for send, `?` for receive, and/or `~` for synchronization).
...
For uses of events by automata (event in the alphabet of the automaton), the circles indicate the type of use: `!` if the automaton sends over the channel, `?` if the automaton receives over the channel, and no text if the automaton <<tut-basics-chapter-synchronizing-events,synchronizes>> over the event (the event is in the alphabet of the automaton).
...
However, for algebraic parameters, an <<tut-values-chapter-types-values-exprs,expression>> may be provided.
...
Good results for <<tools-cif2yed-diagram-model,model diagrams>> can often be obtained by using the btn:[Orthogonal layout].
Select menu:Layout[Orthogonal > Classic] to show the 'Orthogonal Layout' window.
Click the btn:[Dock] button to dock the window.
Once docked, select btn:[Integrated] for btn:[Edge Labeling] and btn:[Free] for btn:[Edge Label Model].
Also enable the btn:[Length Reduction] and btn:[Consider Node Labels] options.
Press the btn:[Apply changes] button (the green rectangle) at the top of the docked window to apply the changes to the layout.

You may want to try different values for the 'Grid' option, to get more or less spacing between nodes.
...
Good results for <<tools-cif2yed-diagram-relations,relations diagrams>> can often be obtained using the 'One-Click Layout' functionality of yEd.
...
The _Structure View_ can be used to see and navigate the structure of the CIF model.
By switching from 'Text' to 'Description' (see upper right corner of the _Structure View_ window), useful descriptions are shown for all elements.
