...
+
When measuring performance, always perform multiple measurements, and take the average.
Furthermore, to ensure the best result, let Java link:https://www.eclipse.org/escet/{escet-website-version}/performance/tweak-perf-settings.html#monitoring-eclipse-heap-status[perform a garbage collection] before starting the data-based synthesis tool.
This ensures that synthesis starts with 'clean' memory.
This is particularly relevant if synthesis is executed from the Eclipse ESCET IDE, to ensure that any previous syntheses do not affect the measurement.
In such cases, do not perform multiple syntheses in parallel, and make sure the Eclipse ESCET IDE is not performing any other operations.
More stable measurements may be obtained by executing synthesis via a <<tools-scripting-chapter-intro,ToolDef script>>.
Here too, perform a garbage collection before each synthesis, especially if multiple syntheses are performed.
A garbage collection may be performed in a ToolDef script as follows:
+
```tooldef
