...
Some are enabled by default, while others are not.
Statistics can be enabled or disabled using the _Statistics_ option (see the <<tools-datasynth-options,options section>>).
The following statistics are available:

* _BDD garbage collection_ [`bdd-gc-collect`] (disabled by default)
+
...
* _BDD node table resize_ [`bdd-gc-resize`] (disabled by default)
+
...
* _BDD cache_ [`bdd-perf-cache`] (disabled by default)
+
...
* _Continuous BDD performance statistics_ [`bdd-perf-cont`] (disabled by default)
+
...
* _Controlled system states_ [`ctrl-sys-states`] (enabled by default)
+
...
* _Maximum used BDD nodes_ [`bdd-perf-max-nodes`] (disabled by default)
+
...
* _Timing_ [`timing`] (disabled by default)
+
...
* _Maximum used memory_ [`max-memory`] (disabled by default)
+
...
For instance, use `--stats=bdd-gc-collect,bdd-gc-resize` to enable both BDD garbage collection statistics and BDD node table resize statistics, but disable all other statistics.
Prefix a name with `pass:c[+]` to add it on top of the defaults, or with `-` to remove it from the defaults.
For instance, use `--stats=+bdd-gc-collect,+bdd-gc-resize` to enable both BDD garbage collection statistics and BDD node table resize statistics, in addition to the default statistics.
