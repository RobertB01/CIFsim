...
The dependencies of an edge are those other edges that may have become enabled by using it.
...
=== Configuring the fixed/initial order

The order in which the edges are considered for when a fixed order is used, is determined by the _Edge order for backward reachability_ and _Edge order for forward reachability_ options (see the <<tools-datasynth-options,options section>>).
This order is also used as the initial order for the workset algorithm.
...
=== Workset algorithm performance

The workset algorithm has two aspects that have significant impact on synthesis performance:

**1) Edge selection heuristic**

Currently, the first edge in the workset is selected.
The configured initial order of the edges thus has an effect on the selection.
Better heuristics will be added in the future.

**2) Edge dependencies**

For each edge, its forward dependencies are those edges that may become enabled after using it.
The backward dependencies are inverted with respect to the forward ones.

Ideally, we would compute the exact dependencies, but with arbitrary guard predicates and arbitrary assigned values in updates, this is typically too complex to analyze statically.
Computing it during the reachability is also not an option, as it would be as expensive as the reachability computation itself.
Hence, we compute an over-approximation.
The larger the over-approximation, the more edges are needlessly applied, impacting synthesis performance.
The dependencies must never be under-approximated, as that would make the workset algorithm invalid, since it then may no longer reach all reachable states.

Currently, each edge has all (other) edges as its dependencies.
This is trivially correct, but this does not lead to good performance.
