...
For basic configuration, the initial order of the boolean/BDD variables can be configured using the _BDD variable order_ option (see the <<tools-datasynth-options,options section>>).
By default, the sorted order is used as initial variable order.
The following predefined initial orders can be used:

[[tools-datasynth-var-order-initial-orders-model]]
...
[[tools-datasynth-var-order-initial-orders-sorted]]
...
[[tools-datasynth-var-order-initial-orders-random]]
