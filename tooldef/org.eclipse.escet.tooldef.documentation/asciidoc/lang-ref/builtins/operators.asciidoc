...
The signatures of the operators are given using tool headers, to show the name
...
`not true`, `-5`), while operators with two arguments are put between the
arguments (e.g. `true and false`, `+1 + 3+`).
...
`right` argument is only evaluated if necessary, i.e. only if the `left`
...
Returns the unary plus of an integer, long, or double value. This is
essentially the
link:http://en.wikipedia.org/wiki/Identity_function[identity function].
...
Returns the addition of two integer, long, or double numbers, the concatenation
...
* If the operation results in overflow (for integer, long, and double numbers
only).
...
Returns the subtraction of two integer, long, or double numbers, the set
...
keys from the second map removed from it.
...
`right`::
The second integer, long, or double number, the (second) set, the list, or
the second map.
...
Returns whether the first integer, long, or double number is less than the
second integer, long, or double number.
...
`true` if the first number is less than the second number, `false`
otherwise.
...
Returns whether the first integer, long, or double number is less than or equal
to the second integer, long, or double number.
...
Returns whether the first integer, long, or double number is greater than the
second integer, long, or double number.
...
`true` if the first number is greater than the second number, `false`
otherwise.
...
Returns whether the first integer, long, or double number is greater than or
equal to the second integer, long, or double number.
...
`true` if the first value is unequal to the second value, `false`
otherwise.
