...
** Please start each commit message with the issue number, e.g. `#1 Commit summary.`.
...
** You may optionally select _Reviewers_, etc.
...
You can address the feedback yourself, by committing and pushing additional commits to the source branch of the merge request.
These will then be automatically picked up by GitLab.
