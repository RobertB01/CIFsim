...
Typically all open issues are assigned to the current work-in-progress version.
During the work on that version, issues that can't be resolved before the final release of that version are moved to the next version.
...
A contributor can discuss with the project committers through issue comments, and they can assign the contributor to the issue.
...
* Project committers working on an issue in a a branch must commit regularly, to allow the other project committers and the rest of the community to see what they're working on.
