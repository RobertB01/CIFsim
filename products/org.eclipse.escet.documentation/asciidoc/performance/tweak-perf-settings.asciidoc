...
The JVM has
link:https://www.oracle.com/java/technologies/javase/vmoptions-jsp.html[way too many options]
...
after each garbage collection operation.
...
generation. In the command line syntax, the `8` is specified.
...
survivor space. In the command line syntax, the `8` is specified.
...
settings for the launcher that starts Eclipse, rather than to the JVM.
...
to the `eclipse.ini` file `-vmargs` instead of replacing them.
...
VisualVM is a tool to monitor, troubleshoot, and profile running Java
applications. It can be downloaded from the
link:https://visualvm.github.io/[VisualVM website].

Download the 'Standalone' version, and extract the archive somewhere on your
system.

On Windows, start `visualvm.exe` from the `bin` directory by double
clicking on it. On Linux, start `visualvm` from the `bin` directory.
On macOS, use the `.dmg` file as you would any other such file.

After you start VisualVM for the first time, you may see some dialogs. Just go
through the steps until you get to the actual application.

In VisualVM, you'll see the currently running Java applications, for the local
system:
