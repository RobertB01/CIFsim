...
Eclipse will ask you to choose a workspace directory when you start the Eclipse ESCET IDE for the <<download-install-first-start,first time>>.
...
Alternatively, in the menu:Show view[] dialog, enter the name of the view (or the first part of it) in the filter box at the top of the dialog, and observe how views that don't match the filter are no longer displayed.
