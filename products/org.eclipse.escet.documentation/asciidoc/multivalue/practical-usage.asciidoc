...
As you typically work with variables in several variations, like `i` and `i+` in the explanation above, this has to be defined first.
...
After creating an instance providing the number of variations that you have, add the variables as you like.
...
The elementary function is `addVariable(<variable>, <variation>)` which adds a node level for variable `<variable>` and usage index `<variation>` (running from `0` to the number of variations excluding the upper bound).

As you usually want to have all variations for a variable, and often want them on consecutive node levels in the tree, `addVariable(<variable>)` adds all variations in one call.
For a list of variables, `addVariablesGroupOnVariables(<list-variables>)` does the same for each variable in the list.
First `N` variations for the first variable, then `N` variations for the second variable, and so on.
If you want the same variation near each other instead, `addVariablesGroupOnSubIndex(<list-variables>)` exists.
...
Modifying a `Node` object is not possible, but you can create a new (updated) object and store that, similar to for example `Integer` objects in the standard Java library.
