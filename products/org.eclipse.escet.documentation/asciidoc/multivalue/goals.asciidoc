...
Most software libraries in this area aim for maximum storage and efficiency,
...
This library uses multi-value decision diagrams (MDDs) instead, and attempts to
keep details of handling nodes away from the application programmer. The
advantage of these choices is that using the library is simpler, in particular
...
As a result, this library works well for applications that need to manipulate
