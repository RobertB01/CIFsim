...
Its syntax is described in the link:https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/Properties.html#load(java.io.Reader)[Java 11 API Specification].
...
| `0.31` +
`10`
| Real value, usually with '`pixel`' unit.
Note that as the output medium is limited in handling fractions, best results are usually obtained with integer values only.
...
There are four flags for checking and debugging diagrams.
...
| Boolean
| Dump coordinates of the graphical elements and child positions within each element.

| `debug.abs_coordinates`
| Boolean
| Dump coordinates of the graphical elements and child positions in the entire diagram.
|===

The `debug.structure` flag gives a short overview of the structure.
The other dump flags use the same structure of output.

The `debug.rel_coordinates` dumps coordinates of each single element.
It is mostly useful for checking correctness of an element type.
The `debug.abs_coordinates` is almost the same, except it dumps coordinates relative to the top-left corner of the diagram, enabling checking positions between different elements.
