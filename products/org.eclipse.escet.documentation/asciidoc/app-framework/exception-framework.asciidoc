...
The application framework contains the exception framework. Its main goal
is to hide stack traces from end users. Exceptions can generally be divided
into two categories: internal errors, and end-user errors.
...
All exceptions that should be presented to the end user are considered end-user
...
implement the `org.eclipse.escet.common.app.framework.exceptions.EndUserException`
interface, and may inherit from the
`org.eclipse.escet.common.app.framework.exceptions.ApplicationException` class.
...
recommended to reuse existing application framework exceptions whenever possible.
...
All exceptions that are not to be presented to end users are considered to
...
the OSGi framework is running, the available plug-ins etc.
...
Java supports the concept of _chained exceptions_. The end-user exceptions of
...
is printed to the console, prefixed with the `ERROR:` text. All the causes of
the exception are printed as well, each on a line of their own. Those messages
are prefixed with the `CAUSE:` text. For exceptions that provide an end-user
readable message, only that message is printed after the `CAUSE:` text. For
other exceptions, the simple name of the exception class, enclosed in
parentheses, is printed between the `CAUSE:` text and the exception message.
...
`org.eclipse.escet.common.app.framework.exceptions.EndUserException` interface)
are considered to provide readable messages. For other exceptions, it is
assumed that they don't. This includes all exceptions provided by Java itself.
...
Developers can enable the development mode option (`DevModeOption` class) to
...
the <<app-framework-option-framework,option framework>> section.

The development mode option is ideal for automated tests, where a stack trace
on stderr is much more ideal than a crash report.
