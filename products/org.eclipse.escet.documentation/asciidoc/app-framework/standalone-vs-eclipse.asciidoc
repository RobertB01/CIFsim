...
One of the goals of the application framework is to make it easier to allow
...
application framework handles them.
...
and incremented when needed, the first instance of the application will run
just fine. Variable `count` starts at zero, and is incremented over and over
...
zero, thus leading to different results for the application.

The conclusion is that one should be careful to avoid static variables that
hold information specific to an application instance.
...
Applications often have settings, and they are generally passed as command
...
work in both scenarios, all applications should use the option framework.
...
interface, the application framework includes an I/O framework.
...
The Eclipse IDE uses the Standard Widget Toolkit (SWT) for its graphical user interface
...
Using the _GUI_ option, the GUI can be enabled or disabled. If disabled,
headless execution mode is used, which disables creation of a SWT display
thread, and thus disables all GUI functionality, including the option dialog.
...
Within Java, the `System.exit` method can be used to immediately terminate
...
such, the `System.exit` method should never be used in applications that
are intended to be executed within the IDE.
...
Stand-alone applications can typically be started from a the command line
terminal window. Pressing kbd:[Ctrl+C] at such a command line terminal window
...
kbd:[Ctrl+C] key combination. Instead, kbd:[Ctrl+C] is used to copy console output
to the clipboard. To remedy this situation, application framework applications
running within Eclipse get a _Terminate_ button with their console within the IDE,
to allow for easy termination.

Furthermore, the application framework allows termination requests via the
`AppEnv.terminate` method. Application framework applications and threads
should regularly call the `AppEnv.isTerminationRequested` method to see
whether they should terminate.

See also the <<use-apps-view-termination,termination features>> of the
<<use-apps-view,Applications>> <<use-terminology-view,view>>.
...
Exceptions are Java feature that allows applications to report error
...
nice error messages, instead of stack traces.

See also the <<app-framework-exception-framework,exception framework>>
section.
...
Java uses system properties (`System.getProperty` method etc). Those
...
applications should use the `+getProp*+` and `+setProp*+` methods in the
`AppEnv` class instead of the property related methods in the `System`
...
One of the standard system properties in Java is the `user.dir` property,
...
should use the methods in the `org.eclipse.escet.common.app.framework.Paths`
class to get and set the current working directory, to resolve relative paths,
etc. These methods also allow both Windows (`\`) and Unix (`/`) separators to
be used in paths, on all platforms, transparently.

Furthermore, within the Eclipse IDE, the concept of a
<<use-terminology-workspace,workspace>> is introduced. In
...
used to load models that are instances of an Ecore. The `Paths` class mentioned
above features methods to create such EMF URIs, from various sources. Those
methods also feature smart handling of `+platform:/auto/...+` paths, an addition to
