...
An element is a rectangular area in the picture that completely displays that node and all its children. An element also has a horizontal connector for linking it with other elements.
Inside an element a collection of graphics (text, quarter arcs, and lines) is created to visualize the flow through the element from the connector at one side to the connector at the other side, non-leaf nodes also incorporate the elements of their children.
The element and its contents is given size and relative positions, thus at the level above in the node tree an element can be seen as a black box with known size and connection points.

Assigning size and relative positions to content in an element is not a trivial process, several graphics are used in a single node, and choice and loop nodes have multiple independent chains that connect again at the extremes of the element.
To avoid complex computations, the generator instead introduces variables for positions and sets equality constraints `a + val = b` (position `a` is exactly `val` units away from position `b`) and/or less-equal constraints `a + val <= b` (position `a` is at least `val` units away from position `b`).
...
At the end of the second step it is know how large the diagram should be.
...
image::{rail-imgsdir}/normal_rule.png[Broken output]

By resizing the image the problems are easier to spot:

image::{rail-imgsdir}/normal_rule_4.png[Enlarged output]

Obviously something is not right here, various lines and arcs are not connected properly to each other.
...
The generator can produce output about how it derives its positions in a diagram, this is controlled by a number of <<rail-debug-properties, debug-properties>> in the properties file.
...
Printed positions in the output should also be the position in the image of the elements if you start counting at `(0, 0)` for the top-left pixel in the image. (Positive X axis to the right, positive Y axis down.)
...
image::{rail-imgsdir}/debug_rule_4.png[Enlarged output]
...
The picture however also has other yellow pixels at the right side of the name boxes, those should not be there, as graphics should not overlap except when connecting an arc to a line.
...
For example, the two pink pixels at the top of the `Identifier` box are from the arcs, they expect the horizontal line to connect there while in reality the line is painted one pixel below that position.
...
In a correctly painted image there should not be pink pixels except at the far left and far right right of leading and trailing lines and around the square box of a non-terminal.
...
In this image those pixels show a problem with the outermost diagram element, at the top-left corner the pixel is at the very edge of the image while at the bottom-right there is are black pixels around it.
The latter shouldn't happen, it points to a bug where the image size is not computed properly.
