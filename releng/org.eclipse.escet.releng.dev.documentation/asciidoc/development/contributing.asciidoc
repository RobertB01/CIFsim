...
This ensures that the build server can link:https://docs.gitlab.com/ee/user/project/merge_requests/allow_collaboration.html#pipeline-status-unavailable-from-mr-page-of-forked-project[communicate the build status] of your merge requests to your forked repository.
It also ensures that your merge requests can be merged into the official Eclipse ESCET GitLab repository:
