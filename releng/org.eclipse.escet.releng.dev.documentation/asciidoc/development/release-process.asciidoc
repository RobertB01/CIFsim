...
** For _Labels_ select _RelEng_ and _Type::Enhancement_.
...
** Manually adapt the version in `org.eclipse.escet.releng.configuration/pom.xml`.
...
`git commit -m "Set standard visible website to release v0.1."`.
...
E.g. use the following to commit the changes:
`git commit -m "Removed milestone and RC websites of release v1.0."`.
...
** Links to `download.eclipse.org` link:https://wiki.eclipse.org/index.php?title=Equinox/p2/p2.mirrorsURL&diff=444984&oldid=370895[should transparently redirect] to `archive.eclipse.org` for archived files.
However, this did not work for P2 update sites in the past.
Hence, this step has been omitted so far, to prevent existing URLs to P2 update sites from breaking for our users.
Once we verify that these issues have been resolved, we can archive older final releases.
We could test this when archiving milestone and release candidate versions.
