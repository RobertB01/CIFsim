...
** If a <<development-release-process-prepare-release,release review is needed>>, create an issue for that as well (`Release review for vN.N`).
...
See e.g. https://www.eclipse.org/lists/technology-pmc/msg12371.html.
...
** End users should however be referred to https://eclipse.org/escet/download.html instead of to `download.eclipse.org`.
...
** Also send an email to the <<development-dev-list-chapter-index,escet-dev list>> with similar content.
...
** For every release (including milestones and release candidates), remove the builds for all previously built tags.
In Jenkins, while logged in, go to link:https://ci.eclipse.org/escet/job/ESCET%20build/view/tags/[].
Select the tag name in the _Name_ column to go the tags page.
