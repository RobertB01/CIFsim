...
By default all text files are opened with their associated text editor that features syntax highlighting, and syntax and type checking.
However, large files are opened by the default non-language-specific text editor, to prevent performance issues.
