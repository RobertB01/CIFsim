...
Alternatively, you can select a Chi file in the _Project Explorer_ tab or _Package Explorer_, and press the kbd:[F9] key.
...
Only building a simulator starts in the same way as above, select a `.chi` source file from the _Project Explorer_ tab, _Package Explorer_ or an editor window, and right-click at it.
