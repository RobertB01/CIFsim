...
Note that length of the string is not the same as the number of characters needed for writing the string literal:
...
With the latter example, not that the `-1` end value in `xs[2:-1:-1]` really means index `-1`, it is *not* rewritten!!
...
They do have binary operators though, which are
...
A tuple expression literal value is written like
...
(See <<ref-run-and-start-statements>> for details on how to create a new process.) An example:
...
A comma separates two columns from each other, a semi colon separates two rows.

The syntax demands at least one semi colon in a literal matrix value, which means you cannot write a matrix literal with a single row directly.
