...
<<tut-figure-producer-consumer>> shows the two processes `P` and `C`, connected by channel variable `a`.
...
The iconic model is given in the previous figure, <<tut-figure-producer-consumer>>.
The model is:
...
The following example gives an illustration, see <<tut-figure-generator-2servers-exit>>.
...
Figure <<tut-figure-sixchilds>> shows a schematic drawing of the children at the table.
+
