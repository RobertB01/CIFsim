...
In this chapter we introduce the concept of _time_, together with the _delay_
statement.
...
steady-state'', he means that time inside the model has progressed a year.
...
The variable `time` denotes the current time in a model. It is a _global_
variable, it can be used in every `model` and `proc`.
The time is a variable of type `real`. Its initial value is `0.0`. The
...
The value of variable `time` can be retrieved by reading from the `time`
variable:
...
The meaning of this statement is that the current time is copied to variable
`t` of type real.

A process delays itself to simulate the processing time of an operation with a
_delay_ statement.
The process postpones or suspends its own actions until the delay ends.

For example, suppose a system has to perform three actions, each action takes
...
An action takes 45 seconds, which is `0.75` time units. The `delay 0.75`
statement represents performing the action, the process is suspended until
0.75 units of time has passed.
...
Adding time to the model allows answering questions about time, often
...
By extending models of the servers with time, time passes while tasks are
...
In this chapter, adding of passing time in a server and how to embed time
...
The model of a deterministic system consists of a deterministic generator, a
deterministic server, and an exit process.
The line is depicted in <<tut-figure2-generator-server-exit>>.

[[tut-figure2-generator-server-exit]]
.Generator `G`, server `S`, and exit `E`
image::{tut-imgsdir}/pictures/generator_server_exit.png[]

Generator process `G` sends items, with constant inter-arrival time `ta`,
via channel `a`, to server process `S`.
The server processes items with constant processing time `ts`, and sends
items, via channel `b`, to exit process `E`.

An item contains a real value, denoting the creation time of the item, for
...
process `E` writes the measurements (the moment in time when the item
...
Parameter `N` denotes the number of items that must flow through the system
...
Process `G` sends an item, with the current time, and delays for `ta`,
before sending the next item to server process `S`.

Server `S` has three parameters, receiving channel `a`, sending channel
`b`, and server processing time `ts`:
...
The process receives an item from process `G`, processes the item during
`ts` time units, and sends the item to exit  process `E`.

Exit `E` has two parameters, receiving channel `a` and the length of the
experiment `N`:
...
The process writes current time `time` and item flow time `time - x` to
the screen for each received item.
Analysis of the measurements will show that the system throughput equals
`1 / ta`, and that the item flow time equals `ts` (if `ta >= ts`).

[[tut-stochastic-system]]
==== A stochastic system

In the next model, the generator produces items with an exponential
...
The model is depicted in <<tut-figure-MM1>>.


[[tut-figure-MM1]]
.Generator `G`, buffer `B`, server `S`, and exit `E`
...
Generator `G` has two parameters, channel variable `a`, and variable
`ta`, denoting the mean inter-arrival time.
An `exponential` distribution is used for deciding the inter-arrival time of
new items:
...
Buffer process `B` is a fifo buffer with infinite capacity, as described at
<<tut-infinite-buffer,An infinite buffer>>.
Server `S` has three parameters, channel
variables `a` and `b`, for receiving and sending items, and a variable for
the average processing time `ts`:
...
The process receives an item from process `G`, processes the item during
`sample u` time units, and sends the item to exit process `E`.

Exit process `E` is the same as previously, see <<tut-servers-deterministic-system>>.
In this case the throughput of the system also equals `1 / ta`, and the
_mean flow_ can be obtained by doing an experiment and analysis of the
resulting measurements (for `ta > ts`).
...
In this section two different types of systems are shown: a serial and a
parallel system.
...
The next model describes a _serial_ system, where an item is processed by one
server, followed by another server.
The generator and the servers are decoupled by buffers.
The model is depicted in <<tut-figure-tandem>>.


[[tut-figure-tandem]]
.A generator, two buffers, two servers, and an exit
...
The various processes are equal to those described previously in
<<tut-stochastic-system>>.


==== Parallel systems

In a parallel system the servers are operating in parallel. Having several
...
<<tut-figure-two-parallel-servers>> depicts the system.

[[tut-figure-two-parallel-servers]]
.A model with two parallel servers
image::{tut-imgsdir}/pictures/two_parallel_servers.png[]

Generator process `G` sends items via `a` to buffer process `B`, and
process `B` sends the items in a first-in first-out manner to the servers
`S`. Both servers send the processed items to the exit process `E` via
channel `c`.
...
To control which server gets the next item, each server must have its own
...
in the buffer.
The model is depicted in <<tut-figure-two-parallel-requesting-servers>>.

[[tut-figure-two-parallel-requesting-servers]]
.A model with two parallel requesting servers
...
In this model, an `unwind` statement is used for the initialization and
running of the two servers.
Via channel `r` an integer value, `0` or `1`, is sent to the buffer.

The items received from generator `G` are stored in list `xs`, the
requests received from the servers are stored in list `ys`.
...
If, there is an item present, _and_ there is a server demanding for an item,
...
Assume the value of `ys[0]` equals `1`, then the expression
`b[ys[0]]!xs[0]`, equals `b[1]!xs[0]`, indicates that the first item of
list `xs`, equals `xs[0]`, is sent to server `1`.

The server first sends a request via channel `r` to the buffer, and waits
for an item. The item is processed, and sent to exit process `E`:
...
assembly processes are often more complicated.

An assembly work station for two components is shown in
<<tut-figure-assembly-two-components>>.

[[tut-figure-assembly-two-components]]
.Assembly for two components
image::{tut-imgsdir}/pictures/assembly_two_components.png[]

The assembly process server `S` is preceded by buffers. The server receives
an item from each buffer `B`, before starting assembly. The received items
...
The process takes a list of channels `c` to receive items from the preceding
buffers.
The output channel `b` is used to send the assembled component away to the
next process.

First, the assembly process receives an item from both buffers. All buffers
...
with channel `c[0]` and stored in `v[0]` in the first alternative. The
...
A generalized assembly work station for `n` components is depicted in
<<tut-figure-assembly-n-components>>.

[[tut-figure-assembly-n-components]]
.Assembly for `n` components, with `m = n - 1`
image::{tut-imgsdir}/pictures/assembly_n_components.png[]

The entire work station (the combined buffer processes and the assembly server
...
The size of the list of channels `a` is determined during initialization of
the workstation.
This size is used for the generation of the process buffers, and the
accompanying channels.

The assembly server process works in the same way as before, except for a
generic `n` components, it is impossible to write a select statement
explicitly. Instead, an _unwind_ is used to unfold the alternatives:
...
channel `c[i]`. The indices of the channels that have not provided an item
are in the list `rec`. Initially, it contains all channels
`0` ... `size(c)`, that is, `range(size(c))`. While `rec` still has a
channel index to monitor, the `unwind i in rec` unfolds all alternatives
that are in the list. For example, if `rec` contains `[0, 1, 5]`,
the `+select unwind i in rec ... end+` is equivalent to:
...
After receiving an item, the index of the channel is removed from `rec` to
...
`delay`, followed by sending the assembled component away with `b!v`.

In practical situations these assembly processes are performed in a more
cascading manner. Two or three components are 'glued' together in one assemble
process, followed in the next process by another assembly process.


=== Exercises

. To understand how time and time units relate to each other, change the time
unit of the model in <<tut-the-clock>>.

.. Change the model to using time units of one second (that is, one time
unit means one second of simulated time).

.. Predict the resulting throughput and flow time for a deterministic case
like in
<<tut-servers-with-time>>, with `ta = 4` and `ts = 5`.
Verify the prediction with an experiment, and explain the result.

. Extend the model <<tut-figure-controlled-factory,A controlled factory>>
in <<tut-buffer-exercises,Buffer exercises>>
with a single deterministic server taking
`4.0` time units to model the production capacity of the factory.
Increase the number of products inserted by the generator, and measure the
average flow time for
