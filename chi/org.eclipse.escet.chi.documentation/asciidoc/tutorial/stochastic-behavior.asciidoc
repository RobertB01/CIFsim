...
<<ref-chapter-distributions,Distributions>>.

The following fragment illustrates the use of the random distribution to model
a dice.
...
distribution, in this case using interval `[1, 7)` (inclusive on the left
...
The variable `dice` is an integer distribution, meaning that values drawn
from the distribution are integer numbers. It is assigned an
uniform distribution.
A throw of a dice is simulated with the _operator_ `sample`.
Each time `sample` is used, a new sample value is obtained from the
distribution.
...
same value. They are useful during the development of the model (see below).
...
When developing a model with stochastic behavior, it is hard to verify whether
...
changes that need to be done after verification.

With constant distributions, a stochastic model with sampling of distributions
...
likely to introduce new errors at this stage in the development of the model.

Constant distributions produce the same value `v` with every call of
`sample`. There is one constant distribution for each type of sample value:
...
This distribution returns the integer value `7` with each `sample u`
operation.
...
Discrete distributions return values from a finite fixed set of possible
...
- [[[law-tut]]] Simulation Modeling & Analysis, fourth edition,
by Averill M. Law,
...
In this chapter, the mathematical notion of stochastic distribution is used to
...
have no notion of varying results.

A (pseudo-)random number generator is used to create stochastic results
instead. It starts with an initial _seed_, an integer number (you can give one
...
_pseudo_-random number generator (a true random number generator would never
...
see <<tool-compile-and-simulate,Compile and simulate>>.
At the end of the simulation, the used
initial seed of that simulation is printed for reference purposes.
...
experiments should match exactly.


=== Exercises

. According to the Chi reference manual, for a `gamma` distribution with
parameters `(a, b)`, the mean equals `a * b`.

.. Use a Chi specification to verify whether this is true for at least 3
different pairs of `a` and `b`.

.. How many samples from the distribution are approximately required to
determine the mean up to three decimals accurate?

. Estimate the mean μ and variance σ^2^ of a triangular distribution
`triangle(1.0, 2.0, 5.0)` by simulating 1000 samples. Recall that the
variance σ^2^ of _n_ samples can be calculated by a function like:
+
[source, chi]
----
...
.. What happens, when the drawn sample is equal to the previous drawn
sample?
