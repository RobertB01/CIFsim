...
. Model the system as shown in <<tut-figure-conveyor-system>> where `T` is a conveyor process with a capacity of _at most_ three products and exponentially distributed conveying times with an average of `4.0`.
+
...
. Model the system as shown in <<tut-figure-three-parallel-servers>> with exponentially distributed server processing times with an average of `4.0`.
+
