...
For example, consider the exchange of items from a producer process `P` to a consumer process `C` as shown in <<tut-figure-pc2>>.
...
The result is shown in <<tut-figure-pbc>>.
...
. To study product flow to and from a factory, a setup as shown in <<tut-figure-controlled-factory>> is created.
+
