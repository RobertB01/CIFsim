...
The names of the operators, the symbols in mathematics and the symbols in the language are presented in <<tut-table-boolsymbols>>.
...
The result of the unary `not`, the binary `and` and `or` operators, for two variables `p` and `q` is given in <<tut-table-truthtable>>.
...
If `p = true` and `q = false`, we find for `p or q` the value `true` (third line in <<tut-table-truthtable>>).
...
The relational operators, with well-known semantics, are listed in <<tut-table-reloper>>.
...
<<tut-table-set-operations>> gives the name, the mathematical notation and the notation in the Chi language.
