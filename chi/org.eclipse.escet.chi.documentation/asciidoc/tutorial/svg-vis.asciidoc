...
A Chi simulation often produces large amounts of textual output that you have
...
indexterm:[SVG,W3C]
The link:http://www.w3.org/TR/SVG11/[Scalable Vector Graphics] (SVG) file
format is a widely used, royalty-free standard for two-dimensional vector
graphics, developed by the link:http://www.w3.org/[World Wide Web Consortium]
...
or link:https://inkscape.org/[Inkscape].
Most modern web browsers also support display of SVG images.


=== Visualization

An SVG file has a tree structure; (graphical) elements are drawn in the same
...
The SVG visualization by the Chi simulator exploits this structure. You access
...
screen notices the changes, and updates the displayed image.

By updating the SVG tree every time when the state of the simulation changes,
you can display how a system evolves over time as an animated image.
